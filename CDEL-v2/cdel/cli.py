...
from cdel.config import load_config, load_config_from_path, write_default_config
...
def main() -> None:
    ...
    config_show_p.add_argument("--pretty", action="store_true")
    ...
    sealed_suite_hash_p = sealed_sub.add_parser("suite-hash")
    sealed_suite_hash_p.add_argument("--path", required=True)

    args = parser.parse_args()
    root = Path(args.root).resolve()
    cfg = _load_config(root, args)
    ...
    if args.cmd == "config":
        if args.config_cmd == "show":
            if not args.config:
                raise SystemExit("config show requires --config")
            payload = cfg.data
            if args.pretty:
    ...
    if args.cmd == "sealed":
        ...
        if args.sealed_cmd == "worker":
            ...
            return
        if args.sealed_cmd == "suite-hash":
            from cdel.sealed.suites import compute_suite_hash_path

            path = Path(args.path)
            try:
                digest = compute_suite_hash_path(path)
...
def _load_config(root: Path, args) -> Config:
    if not args.config:
    ...
    try:
        return load_config_from_path(root, config_path)
    except FileNotFoundError as exc:
        raise SystemExit(f"config not found: {config_path}") from exc
    except OSError as exc:
        raise SystemExit(f"config unreadable: {config_path}: {exc}") from exc
...
def _main_entry() -> int:
    try:
        ...
        print(f"cdel error: {exc}", file=sys.stderr)
        return 1
