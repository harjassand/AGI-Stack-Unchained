"""Sealed architecture eval worker (dev) (v11.0)."""
...
from .fixed_q32_v1 import Q, q32_obj, q32_from_ratio, q32_div, iroot2_floor, iroot4_floor
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="sealed_arch_eval_worker_v1")
    ...
    if eval_config.get("schema_version") != "arch_eval_config_dev_v1":
    ...
    stdout_payload = f"sealed_eval_dev_ok:{manifest.get('arch_id')}\n"
    stderr_payload = "sealed_eval_dev_trace:eval\n"
    print(stdout_payload, end="")
    print(stderr_payload, end="", file=sys.stderr)

    receipt = {
        "schema_version": "sas_model_eval_receipt_v1",
