"""Sealed conjecture generation worker (v11.1)."""
...
from .sas_conjecture_generator_v1 import generate_conjectures
from .sas_conjecture_ir_v1 import compute_conjecture_id, compute_fingerprint, compute_metrics, render_statement, validate_conjecture_ir
...
def _run_lean_check(
    *,
    ...
    proof_path = work_dir / "proof.lean"
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="sealed_sas_conjecture_gen_worker_v1")
    ...
    dev_count = int(config.get("dev_count", max_conjectures))
    heldout_count = int(config.get("heldout_count", 0))
    simp_lemmas = [str(x) for x in (config.get("simp_lemmas") or [])]
    ...
    if len(conjectures) < max(1, dev_count + heldout_count):
        raise SystemExit("not enough conjectures")

    bundle_conjectures = []
    for conj in conjectures[: max_conjectures]:
        validate_conjecture_ir(conj)
        conjecture_id = compute_conjecture_id(conj)

        # Write IR
        ir_path = ir_dir / f"sha256_{conjecture_id.split(':',1)[1]}.sas_conjecture_ir_v1.json"
        ...
        for method in ["rfl", "simp"]:
            if method == "simp":
                lemmas = ", ".join(simp_lemmas)
                tactic = f"simp [{lemmas}]" if lemmas else "simp"
            else:
                tactic = "rfl"
            proof_text = f"{statement_text} by {tactic}\n"
        ...
        bundle_conjectures.append(
            {
                ...
                "status": status,
                "rejection_reason": "TRIVIAL_SOLVED" if status == "TRIVIAL_REJECTED" else "",
            }
        )
    ...
    bundle_path = bundles_dir / f"sha256_{bundle_hash.split(':',1)[1]}.sas_conjecture_bundle_v1.json"
    ...
    receipt_path = receipts_dir / f"sha256_{receipt_hash.split(':',1)[1]}.sas_conjecture_gen_receipt_v1.json"
