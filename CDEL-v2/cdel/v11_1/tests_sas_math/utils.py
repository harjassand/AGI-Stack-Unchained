...
from cdel.v11_1.sas_conjecture_ir_v1 import compute_conjecture_id, compute_fingerprint as compute_conj_fingerprint, compute_metrics, render_statement
from cdel.v11_1.sas_conjecture_seed_v1 import compute_conjecture_seed
...
def _write_root_manifest(state_dir: Path, agi_root_raw: str) -> Path:
    canon = canon_root_v1_for(agi_root_raw, "rsi_sas_math_v11_1")
    manifest = dict(canon)
    manifest.update(
        {
...
def build_state(
    ...
    *,
    ...
    sas_root = agi_root / "daemon" / "rsi_sas_math_v11_1"
    ...
    pack = {
        ...
        "conjecture_gen_config_path": "sas_conjecture_gen_config_v1.json",
        "conjecture_selection_policy_path": "sas_conjecture_selection_policy_v1.json",
        "thresholds": {
    ...
    # Conjecture generation config + selection policy
    conjecture_cfg = {
        "schema_version": "sas_conjecture_gen_config_v1",
        "max_conjectures": 1,
        "dev_count": 1,
        "heldout_count": 0,
        "max_depth": 4,
        "max_node_count": 12,
        "simp_lemmas": ["Nat.add_zero"],
        "time_limit_ms": 1000,
        "memory_limit_mb": 256,
    }
    write_canon_json(config_dir / "sas_conjecture_gen_config_v1.json", conjecture_cfg)

    selection_policy = {
        "schema_version": "sas_conjecture_selection_policy_v1",
        "weights": {"node_count": 1, "binder_count": 1, "depth": 1},
    }
    write_canon_json(config_dir / "sas_conjecture_selection_policy_v1.json", selection_policy)
    ...
    conj_ir = {
        ...
        "goal": {"op": "Eq", "lhs": {"op": "Add", "args": [{"op": "Var", "name": "n"}, {"op": "NatLit", "value": 0}]}, "rhs": {"op": "Var", "name": "n"}},
    }
    conj_ir["conjecture_id"] = compute_conjecture_id(conj_ir)
    conj_id = conj_ir["conjecture_id"]
    write_canon_json(state_dir / "conjectures" / "ir" / f"sha256_{conj_id.split(':',1)[1]}.sas_conjecture_ir_v1.json", conj_ir)
    ...
    bundle = {
        "schema_version": "sas_conjecture_bundle_v1",
        "bundle_id": "",
    ...
    write_canon_json(state_dir / "conjectures" / "bundles" / f"sha256_{bundle_hash.split(':',1)[1]}.sas_conjecture_bundle_v1.json", bundle)

    receipt = {
        "schema_version": "sas_conjecture_gen_receipt_v1",
        "receipt_id": "",
        "created_utc": "2026-02-04T00:00:00Z",
        "generator_version": "sas_conjecture_gen_v1",
    ...
    write_canon_json(state_dir / "conjectures" / "receipts" / f"sha256_{receipt_hash.split(':',1)[1]}.sas_conjecture_gen_receipt_v1.json", receipt)

    # Selection receipt
    sel_policy_hash = sha256_prefixed(canon_bytes(selection_policy))
    score = int(metrics["node_count"]) + int(metrics["binder_count"]) + int(metrics["depth"])
    selection = {
        "schema_version": "sas_conjecture_selection_receipt_v1",
        "receipt_id": "",
        "created_utc": "2026-02-04T00:00:00Z",
        ...
        "selection_metrics": {
            ...
            "binder_count": int(metrics["binder_count"]),
    ...
    write_canon_json(
        state_dir / "conjectures" / "selection" / f"sha256_{selection_hash.split(':',1)[1]}.sas_conjecture_selection_receipt_v1.json",
        selection,
    )
