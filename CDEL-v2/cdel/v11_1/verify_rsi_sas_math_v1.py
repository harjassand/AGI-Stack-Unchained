"""Verifier for RSI SAS-MATH v11.1."""
...
from .fixed_q32_v1 import Q, Q32Error, parse_q32, q32_from_ratio
from .path_canon_v1 import canon_root_v1_for
from .sas_conjecture_ir_v1 import (
...
from .sas_conjecture_seed_v1 import compute_conjecture_seed
from .sas_math_eval_v1 import compute_eval_report
from .sas_math_fingerprint_v1 import compute_fingerprint
from .sas_math_ledger import SAS_MATH_EVENT_TYPES, load_ledger, validate_chain
...
def _verify_root_manifest(state_dir: Path) -> None:
    ...
    expected = canon_root_v1_for(str(manifest.get("agi_root_raw", "")), "rsi_sas_math_v11_1")
...
def _validate_attempt_receipt(state_dir: Path, receipt: dict[str, Any], toolchains: dict[str, dict[str, Any]]) -> None:
    ...
    # Attempt record with NETWORK_NONE
    record_dir = state_dir / "math" / "attempts" / "records"
    if record_dir.exists():
        # optional: ensure at least one record matches attempt_id and contains NETWORK_NONE
...
def verify(state_dir: Path, *, mode: str) -> dict[str, Any]:
    ...
    bundle_paths = list((conj_dir / "bundles").glob("sha256_*.sas_conjecture_bundle_v1.json"))
    if not bundle_paths:
        _fail("CONJECTURE_BUNDLE_MISSING")
    bundle = _load_json(bundle_paths[0])
    _require_schema(bundle, "sas_conjecture_bundle_v1")
    ...
    receipt_paths = list((conj_dir / "receipts").glob("sha256_*.sas_conjecture_gen_receipt_v1.json"))
    if not receipt_paths:
        _fail("CONJECTURE_GEN_RECEIPT_MISSING")
    gen_receipt = _load_json(receipt_paths[0])
    _require_schema(gen_receipt, "sas_conjecture_gen_receipt_v1")
    ...
    gen_cfg = _load_json(config_dir / "sas_conjecture_gen_config_v1.json")
    sel_policy = _load_json(config_dir / "sas_conjecture_selection_policy_v1.json")
    ...
    for item in bundle.get("conjectures") or []:
        ...
        ir_path = conj_dir / "ir" / f"sha256_{conj_id.split(':',1)[1]}.sas_conjecture_ir_v1.json"
        ...
        status = "TRIVIAL_REJECTED" if any_pass else "ACCEPTED"
    ...
    # Selection receipt
    sel_paths = list((conj_dir / "selection").glob("sha256_*.sas_conjecture_selection_receipt_v1.json"))
    if not sel_paths:
        _fail("CONJECTURE_SELECTION_RECEIPT_MISSING")
    selection = _load_json(sel_paths[0])
    _require_schema(selection, "sas_conjecture_selection_receipt_v1")
    ...
    if int(sel_metrics.get("node_count", -1)) != int(metrics.get("node_count", -2)):
        _fail("CONJECTURE_SELECTION_METRICS_MISMATCH")
    if int(sel_metrics.get("binder_count", -1)) != int(metrics.get("binder_count", -2)):
        _fail("CONJECTURE_SELECTION_METRICS_MISMATCH")
    if int(sel_metrics.get("depth", -1)) != int(metrics.get("depth", -2)):
        _fail("CONJECTURE_SELECTION_METRICS_MISMATCH")
    weights = (sel_policy.get("weights") or {}) if isinstance(sel_policy, dict) else {}
    score = int(metrics.get("node_count", 0)) * int(weights.get("node_count", 1))
    score += int(metrics.get("binder_count", 0)) * int(weights.get("binder_count", 1))
    score += int(metrics.get("depth", 0)) * int(weights.get("depth", 1))
    if int(sel_metrics.get("score", 0)) != int(score):
        _fail("CONJECTURE_SELECTION_METRICS_MISMATCH")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify RSI SAS-MATH v11.1")
