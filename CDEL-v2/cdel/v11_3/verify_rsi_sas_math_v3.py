"""Verifier for RSI SAS-MATH v11.3."""
...
from .sas_conjecture_seed_v3 import compute_conjecture_seed
from .sas_conjecture_selection_v3 import compute_score, select_conjecture
...
def _load_constants() -> dict[str, Any]:
    path = _repo_root() / "meta-core" / "meta_constitution" / "v11_3" / "constants_v1.json"
...
def _verify_root_manifest(state_dir: Path) -> None:
    ...
    expected = canon_root_v1_for(str(manifest.get("agi_root_raw", "")), "rsi_sas_math_v11_3")
...
def _contains_forbidden_statement(text: str, tokens: list[str]) -> bool:
    stripped = _strip_lean_comments(text)
    return _contains_forbidden(stripped, tokens)
...
def _count_tactic_keywords(text: str, keywords: list[str]) -> int:
    total = 0
    for key in keywords:
        if not key:
            continue
        total += len(re.findall(rf"\b{re.escape(key)}\b", text))
    return total


def _proof_metrics(text: str, keywords: list[str]) -> dict[str, Any]:
    lines = [line for line in text.splitlines() if line.strip()]
    line_count = len(lines)
    keyword_count = _count_tactic_keywords(text, keywords)
    has_induction = re.search(r"\binduction\b", text) is not None
    has_cases = re.search(r"\bcases\b", text) is not None
    return {
        "proof_line_count": line_count,
        "tactic_keyword_count": keyword_count,
        "has_induction_or_cases": bool(has_induction or has_cases),
    }


def _enforce_allowlist(policy_ir: dict[str, Any], allowlist: dict[str, Any]) -> None:
    schema = str(allowlist.get("schema_version") or "")
    if schema == "sas_math_policy_allowlist_v2":
        forbidden = [str(x) for x in (allowlist.get("forbidden_substrings") or [])]
        for tac in policy_ir.get("lean_tactics") or []:
            if _contains_forbidden(str(tac), forbidden):
                _fail("ALLOWLIST_VIOLATION")
        if policy_ir.get("toy_checker_proofs"):
            _fail("ALLOWLIST_VIOLATION")
        return
...
def verify(state_dir: Path, *, mode: str) -> dict[str, Any]:
    ...
    allowlist = _load_json(config_dir / "sas_math_policy_allowlist_v2.json")
    _require_schema(allowlist, "sas_math_policy_allowlist_v2")
    ...
    # Preamble
    gen_cfg = _load_json(config_dir / "sas_conjecture_gen_config_v3.json")
    sel_policy = _load_json(config_dir / "sas_conjecture_selection_policy_v3.json")
    ...
    preamble_rel = str(gen_cfg.get("preamble_relpath") or "")
    preamble_path = Path(preamble_rel)
    if not preamble_path.is_absolute():
        preamble_path = _repo_root() / preamble_path
    if not preamble_path.exists():
        _fail("PREAMBLE_MISSING")
    preamble_bytes = preamble_path.read_bytes()
    preamble_hash = sha256_prefixed(preamble_bytes)

    # Forbidden proof tokens
    forbidden_tokens_path = config_dir / "sas_math_forbidden_proof_tokens_v1.json"
    if not forbidden_tokens_path.exists():
        _fail("MISSING_ARTIFACT")
    forbidden_tokens = _load_json(forbidden_tokens_path)
    if not isinstance(forbidden_tokens, dict) or forbidden_tokens.get("schema_version") != "sas_math_forbidden_proof_tokens_v1":
        _fail("SCHEMA_INVALID")
    token_list = [str(x) for x in (forbidden_tokens.get("tokens") or [])]
    forbidden_tokens_hash = sha256_prefixed(canon_bytes(forbidden_tokens))
    ...
    if constants.get("sas_math.policy_allowlist_hash") != allowlist_hash:
        _fail("ALLOWLIST_VIOLATION")
    if constants.get("sas_math.preamble_sha256") != preamble_hash:
        _fail("PREAMBLE_HASH_MISMATCH")
    if constants.get("sas_math.forbidden_proof_tokens_sha256") != forbidden_tokens_hash:
        _fail("FORBIDDEN_TOKEN_PROOF")

    # Conjecture bundle + receipts
    conj_dir = state_dir / "conjectures"
    bundle_paths = list((conj_dir / "bundles").glob("sha256_*.sas_conjecture_bundle_v3.json"))
    if not bundle_paths:
        _fail("CONJECTURE_BUNDLE_MISSING")
    bundle = _load_json(bundle_paths[0])
    _require_schema(bundle, "sas_conjecture_bundle_v3")
    ...
    receipt_paths = list((conj_dir / "receipts").glob("sha256_*.sas_conjecture_gen_receipt_v3.json"))
    if not receipt_paths:
        _fail("CONJECTURE_GEN_RECEIPT_MISSING")
    gen_receipt = _load_json(receipt_paths[0])
    _require_schema(gen_receipt, "sas_conjecture_gen_receipt_v3")
    ...
    if gen_receipt.get("generator_config_hash") != gen_cfg_hash:
        _fail("CONJECTURE_GEN_CONFIG_DRIFT")
    if gen_receipt.get("generator_version") != "sas_conjecture_gen_v3":
        _fail("SCHEMA_INVALID")
    ...
    forbidden_stmt_tokens = [str(x) for x in (gen_cfg.get("forbidden_tokens_statement") or [])]
    ...
    for item in bundle.get("conjectures") or []:
        ...
        fp_hash = item.get("fingerprint_hash")
        if not isinstance(fp_hash, str):
            _fail("CONJECTURE_FINGERPRINT_MISMATCH")
        if fp_hash in seen_fingerprints:
            _fail("DUPLICATE_FINGERPRINT_IN_BUNDLE")
        seen_fingerprints.add(fp_hash)

        ir_path = conj_dir / "ir" / f"sha256_{conj_id.split(':',1)[1]}.sas_conjecture_ir_v3.json"
        ...
        if compute_conjecture_id(ir) != conj_id:
            _fail("CONJECTURE_ID_MISMATCH")
        if compute_fingerprint_hash(ir) != fp_hash:
            _fail("CONJECTURE_FINGERPRINT_MISMATCH")
        ...
        statement_text = render_statement(ir, preamble_text=preamble_bytes.decode("utf-8")).rstrip("\n") + "\n"
        ...
        if _contains_forbidden_statement(statement_text, forbidden_stmt_tokens):
            _fail("FORBIDDEN_TOKEN_STATEMENT")

        status = str(item.get("status") or "")
        rejection_reason = str(item.get("rejection_reason") or "")

        # Relevance + recursive focus gates
        used = collect_used_binders(ir.get("goal") or {})
        declared = {v.get("name") for v in (ir.get("vars") or []) if isinstance(v, dict)}
        if status == "CANDIDATE" and not declared.issubset(used):
            _fail("UNUSED_BINDER_ACCEPTED")
        if status == "CANDIDATE" and (not metrics.get("has_lnat") or not metrics.get("has_rec_op")):
            _fail("NO_RECURSIVE_STRUCTURE_ACCEPTED")

        # Triviality checks
        checks = item.get("triviality_checks") or []
        if rejection_reason in {"SYNTAX_TAUTOLOGY", "FORBIDDEN_TOKEN_STATEMENT", "UNUSED_BINDER", "NO_RECURSIVE_STRUCTURE"}:
            ...
            if seen_methods != {"rfl", "simp_preamble"}:
                _fail("CONJECTURE_TRIVIALITY_CHECK_MISSING")
            any_pass = False
            for check in checks:
            ...
            if any_pass:
                if status != "TRIVIAL_REJECTED" or not rejection_reason.startswith("TRIVIAL_SOLVED_"):
                    _fail("CONJECTURE_TRIVIALITY_CHECK_MISSING")
            else:
                if status != "CANDIDATE" or rejection_reason != "":
                    _fail("CONJECTURE_TRIVIALITY_CHECK_MISSING")

        conj_by_id[conj_id] = item

    # Selection receipt
    sel_paths = list((conj_dir / "selection").glob("sha256_*.sas_conjecture_selection_receipt_v3.json"))
    if not sel_paths:
        _fail("CONJECTURE_SELECTION_RECEIPT_MISSING")
    selection = _load_json(sel_paths[0])
    _require_schema(selection, "sas_conjecture_selection_receipt_v3")
    ...
    for key in ["node_count", "binder_count", "depth", "op_counts", "has_lnat", "has_rec_op"]:
        if sel_metrics.get(key) != metrics.get(key):
            _fail("CONJECTURE_SELECTION_METRICS_MISMATCH")
    if sel_metrics.get("score") != compute_score(metrics, "CANDIDATE", ""):
        _fail("CONJECTURE_SELECTION_METRICS_MISMATCH")
    ...
    allowed_tactics = [str(x) for x in (allowlist.get("allowed_tactics") or [])]
    forbidden_substrings = [str(x) for x in (allowlist.get("forbidden_substrings") or [])]

    def _load_proof_text(receipt: dict[str, Any]) -> str:
        proof_hash = receipt.get("proof_artifact_hash")
    ...
    def _depth_gate_pass(text: str) -> bool:
        scan_text = _strip_lean_comments(text)
        if _contains_forbidden(scan_text, forbidden_substrings):
            return False
        metrics = _proof_metrics(text, allowed_tactics)
        if not metrics["has_induction_or_cases"]:
            return False
        if int(metrics["proof_line_count"]) < 14:
            return False
        if int(metrics["tactic_keyword_count"]) < 18:
            return False
        return True

    def _has_forbidden_proof_token(text: str) -> bool:
        for token in token_list:
            if _contains_token_boundary(text, token):
                return True
        return False
    ...
    for rec in candidate_receipts:
        ...
        if rec.get("result") == "PASS":
            proof_text = _load_proof_text(rec)
            if not proof_text:
                _fail("MISSING_ARTIFACT")
            if _has_forbidden_proof_token(proof_text):
                _fail("FORBIDDEN_TOKEN_PROOF")
            gate_ok = _depth_gate_pass(proof_text)
            cand_depth_gate[pid] = gate_ok
            if not gate_ok:
                continue
            cand_pass[pid] = True
    ...
    promo_improved = sorted(promo.get("improved_problem_ids") or [])
    for pid in promo_improved:
        if pid in cand_depth_gate and not cand_depth_gate.get(pid, False):
            _fail("DEPTH_GATE_VIOLATION")
    if not improved:
        _fail("NO_IMPROVEMENT")

    if promo_improved != improved:
    ...
    for pid in improved:
        ...
        if base_rec is None or cand_rec is None:
            _fail("IMPROVEMENT_MISMATCH")
        if not cand_depth_gate.get(pid, False):
            _fail("DEPTH_GATE_VIOLATION")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify RSI SAS-MATH v11.3")
