...
from cdel.v1_7r.canon import load_canon_json, write_canon_json, sha256_prefixed
from cdel.v12_0.verify_rsi_sas_code_v1 import verify

from .utils import build_state


def test_preamble_bubblesort_cannot_alias_mergesort(tmp_path: Path) -> None:
    ...
    try:
        if "bubbleIter xs.length xs" not in original:
            raise AssertionError("expected bubbleSort body to contain bubbleIter")
        tampered = original.replace("bubbleIter xs.length xs", "mergeSort xs")
