...
def test_preamble_forbidden_builtin_sort_rejected(tmp_path: Path) -> None:
    ...
    try:
        tampered = original + "\n\n" + "def _forbidden_merge : List Nat -> List Nat := List.mergeSort\n"
        ...
        assert "INVALID:PREAMBLE_FORBIDDEN_TOKEN:List.mergeSort" in str(exc.value)
    finally:
