...
def test_verifier_rejects_identity_sort(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    state = build_state(tmp_path)

    def fake_execute(algo_kind: str, xs: list[int]) -> tuple[list[int], dict[str, int]]:
        # Identity output (unsorted for many cases).
        return list(xs), {}
    ...
    assert "INVALID:NOT_SORTED" in str(exc.value)
