...
from cdel.v18_0.tests_omega_daemon.utils import latest_file, load_json, pack_path, repo_root, run_tick_with_pack


def _find_matching_artifact(root: Path, source: dict[str, object], *, run_scope_root: Path | None = None) -> Path:
    ...
    if run_scope_root is not None:
        ...
        if producer_run_id:
            ...
            if scoped_dir.is_dir():
                candidates.extend(sorted(scoped_dir.glob(f"**/{filename}")))
            if not candidates:
                candidates.extend(sorted(scope.glob(f"**/{producer_run_id}/**/{filename}")))
        if not candidates:
...
def test_verifier_recomputes_observation_with_stats_source(tmp_path, monkeypatch) -> None:
    ...
    try:
        ...
        snapshot_path = latest_file(state_dir_2 / "snapshot", "sha256_*.omega_tick_snapshot_v1.json")
        snapshot = load_json(snapshot_path)
        obs_hash = str(snapshot["observation_report_hash"]).split(":", 1)[1]
        observation_path = state_dir_2 / "observations" / f"sha256_{obs_hash}.omega_observation_report_v1.json"
        observation = load_json(observation_path)
        sources = observation.get("sources")
        assert isinstance(sources, list) and sources
        assert any(isinstance(row, dict) and row.get("schema_id") == "omega_tick_stats_v1" for row in sources)
