...
def test_conquer_uses_refinery_cache_when_present(monkeypatch, tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    ...
    train_sha256 = str((split or {}).get("train_sha256", ""))

    config_id = "text_word_unigram|alpha=1.0"
    proposal_no_id = {
        "schema_version": "polymath_refinery_proposal_v1",
        "domain_id": info["domain_id"],
        "train_sha256": train_sha256,
        "config_id": config_id,
        "config": {"token_family": "text_word_unigram", "alpha_num_u64": 10, "alpha_den_u64": 10},
        "val_metric_q32": 1,
        "model_complexity_u64": 1,
        "baseline_val_metric_q32": 0,
        "expected_val_delta_q32": 1,
    }
    proposal_id = canon_hash_obj(proposal_no_id)
    proposal_payload = dict(proposal_no_id)
    proposal_payload["proposal_id"] = proposal_id
    proposal_path = store_root / "refinery" / "proposals" / f"{proposal_id}.polymath_refinery_proposal_v1.json"
    write_canon_json(proposal_path, proposal_payload)
    write_jsonl_line(
        store_root / "refinery" / "indexes" / "domain_train_to_best.jsonl",
        {
    ...
    assert str(report.get("selected_config_id", "")) == config_id
