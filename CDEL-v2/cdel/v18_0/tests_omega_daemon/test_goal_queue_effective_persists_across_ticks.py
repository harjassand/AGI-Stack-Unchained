...
def test_goal_queue_effective_persists_across_ticks(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(
        coordinator_v1,
    ...
    effective_payload["goals"].append(
        {
            "goal_id": "goal_test_effective_overlay_0001",
    ...
    state_dir_2 = _state_dir(run_root)
    state_payload = None
    for row in sorted((state_dir_2 / "state").glob("sha256_*.omega_state_v1.json")):
