...
def _make_dispatch_ctx(tmp_path: Path) -> tuple[dict[str, object], Path, Path]:
    run_root = tmp_path / "runs" / "arena_patch_route_tick_0001"
...
def _write_min_bundle(subrun_root: Path, winner_candidate_id: str) -> tuple[Path, str]:
    promo_dir = (subrun_root / "promotion").resolve()
    ...
    path, digest = _write_hashed_json(promo_dir, "omega_promotion_bundle_ccap_v1.json", payload)
    return path, digest
...
def _fake_promoted_receipt(bundle_hash: str) -> dict[str, object]:
    return {
        ...
        "meta_core_verifier_fingerprint": {
            "constitution_meta_hash": "sha256:" + ("b" * 64),
            "binary_hash_or_build_id": "sha256:" + ("c" * 64),
        },
        "result": {"status": "PROMOTED", "reason_code": None, "route": "ACTIVE"},
        "active_manifest_hash_after": "sha256:" + ("d" * 64),
    }


def test_promoter_routes_patch_winner_to_ccap_v1(tmp_path: Path, monkeypatch) -> None:
    dispatch_ctx, _dispatch_dir, subrun_root = _make_dispatch_ctx(tmp_path)
    ...
    monkeypatch.setattr(promoter_v19, "_write_utility_proof_receipt", lambda **_: ({}, "sha256:" + ("e" * 64)))

    calls = {"v18_run_promotion": 0}

    def _fake_v18_run_promotion(**_: object) -> tuple[dict[str, object], str]:
        calls["v18_run_promotion"] += 1
        return _fake_promoted_receipt(bundle_hash), "sha256:" + ("f" * 64)
    ...
    assert str((receipt.get("result") or {}).get("status", "")) == "PROMOTED"
    assert str(receipt.get("result_kind", "")) == "PROMOTED_COMMIT"
