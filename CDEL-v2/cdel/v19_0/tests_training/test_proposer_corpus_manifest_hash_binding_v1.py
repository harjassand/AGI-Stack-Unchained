...
def _patch_text(idx: int) -> str:
    relpath = "campaigns/hash_binding.py"
    return (
...
def _make_tick(*, runs_root: Path, run_name: str, idx: int, outcome: str, reason_code: str | None) -> None:
    ...
    candidate_payload = {
        ...
        "declared_touched_paths": ["campaigns/hash_binding.py"],
        "derived_touched_paths": ["campaigns/hash_binding.py"],
    ...
    bundle_payload = {
        ...
        "touched_paths": ["campaigns/hash_binding.py"],
    ...
    promotion_receipt = {
        ...
        "result": {"status": outcome, "reason_code": reason_code, "route": "ACTIVE"},
...
def _blob_path(out_root: Path, blob_id: str) -> Path:
...
def _filename_digest(path: Path) -> str:
    ...
    digest = name[len(prefix) :].split(".", 1)[0]
    return f"sha256:{digest}"


def test_proposer_corpus_manifest_hash_binding_v1(tmp_path: Path) -> None:
    runs_root = tmp_path / "runs"
    out_root = tmp_path / "out" / "daemon" / "proposer_models" / "datasets"

    _make_tick(runs_root=runs_root, run_name="run_promoted", idx=1, outcome="PROMOTED", reason_code=None)
    _make_tick(runs_root=runs_root, run_name="run_rejected", idx=2, outcome="REJECTED", reason_code="NO_BUNDLE_BENCH_FAIL")

    _run_builder(runs_root=runs_root, out_root=out_root)
    manifest = _load_manifest(out_root)

    sft_blob_path = _blob_path(out_root, manifest["sft_examples_blob_id"])
    dpo_blob_path = _blob_path(out_root, manifest["dpo_pairs_blob_id"])

    assert manifest["hashes"]["sft_examples_sha256"] == manifest["sft_examples_blob_id"]
    assert manifest["hashes"]["dpo_pairs_sha256"] == manifest["dpo_pairs_blob_id"]

    assert _hash_bytes(sft_blob_path) == manifest["sft_examples_blob_id"]
    assert _hash_bytes(dpo_blob_path) == manifest["dpo_pairs_blob_id"]

    assert _filename_digest(sft_blob_path) == manifest["sft_examples_blob_id"]
    assert _filename_digest(dpo_blob_path) == manifest["dpo_pairs_blob_id"]
