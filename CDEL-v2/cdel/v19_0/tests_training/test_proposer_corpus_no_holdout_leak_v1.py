...
def _make_fixture(runs_root: Path) -> None:
    run_dir = runs_root / "run_no_leak"
    dispatch_id = "d01"
    ...
    candidate_id = _sha_text("candidate")
    patch = (
        "diff --git a/campaigns/leak_safe.py b/campaigns/leak_safe.py\n"
        "--- a/campaigns/leak_safe.py\n"
        "+++ b/campaigns/leak_safe.py\n"
        "@@ -1 +1 @@\n"
        "-safe_old\n"
        "+safe_new\n"
    )
    ...
    candidate_payload = {
        ...
        "declared_touched_paths": ["campaigns/leak_safe.py"],
        "derived_touched_paths": ["campaigns/leak_safe.py"],
    ...
    run_payload = {
        ...
        "receipt_id": _sha_text("run"),
        "tick_u64": 1,
        "arena_state_out_id": _sha_text("state"),
        "n_generated_u64": 1,
        "n_admitted_u64": 1,
        "n_backlogged_u64": 0,
        "n_considered_u64": 1,
    ...
    selection_payload = {
        ...
        "receipt_id": _sha_text("selection"),
        "inputs_descriptor_id": _sha_text("inputs"),
        "arena_state_in_id": _sha_text("in"),
        "candidates_considered": [{"candidate_id": candidate_id, "score_q32": 1, "cost_q32": 1, "risk_class": "LOW"}],
        "ranked_candidate_ids": [candidate_id],
        "winner_candidate_id": candidate_id,
        "tie_break_proof": {"seed": _sha_text("seed"), "ordered_candidate_ids": [candidate_id], "chosen_candidate_id": candidate_id},
    ...
    bundle_payload = {
        ...
        "touched_paths": ["campaigns/leak_safe.py"],
    ...
    promotion_receipt = {
        ...
        "result": {"status": "PROMOTED", "reason_code": None, "route": "ACTIVE"},
    ...
    bench_payload = {
        ...
        "receipt_id": _sha_text("bench"),
        "ek_id": EK_ID,
        "anchor_suite_set_id": _sha_text("anchor"),
        "extensions_ledger_id": LEDGER_ID,
        "suite_runner_id": _sha_text("runner"),
        "executed_suites": [{"suite_id": _sha_text("suite-row"), "suite_outcome": "PASS", "metrics": {}, "gate_results": [], "budget_outcome": {"within_budget_b": True, "cpu_ms_u64": 1, "wall_ms_u64": 1}}],
        "aggregate_metrics": {},
        "gate_results": [],
...
def _run_builder(*, runs_root: Path, out_root: Path) -> dict[str, Any]:
    cmd = [
        "python3",
...
def _read_blob_text(out_root: Path, blob_id: str) -> str:
...
def test_proposer_corpus_no_holdout_leak_v1(tmp_path: Path) -> None:
    runs_root = tmp_path / "runs"
    out_root = tmp_path / "out" / "daemon" / "proposer_models" / "datasets"
    _make_fixture(runs_root)

    _run_builder(runs_root=runs_root, out_root=out_root)
    manifest = _load_manifest(out_root)

    text_sft = _read_blob_text(out_root, manifest["sft_examples_blob_id"])
    text_dpo = _read_blob_text(out_root, manifest["dpo_pairs_blob_id"])
    merged = text_sft + "\n" + text_dpo

    assert "holdout" not in merged.lower()
    assert "HOLDOUT_" not in merged
