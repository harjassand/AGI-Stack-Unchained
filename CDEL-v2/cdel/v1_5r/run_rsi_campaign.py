...
from .proposals.inbox import INBOX_FAMILY_DIR, INBOX_MACRO_DIR, INBOX_MECH_PATCH_DIR, INBOX_META_PATCH_DIR
...
def _write_base_state(
    ...
    *,
    ...
    # Intentionally do not auto-correct the policy name to the frontier requirement here.
    # Integrity/portfolio real campaigns rely on explicit mech patches to adopt the required name.
...
def _reset_inbox(state_dir: Path) -> None:
    for rel in (INBOX_FAMILY_DIR, INBOX_MACRO_DIR, INBOX_MECH_PATCH_DIR, INBOX_META_PATCH_DIR):
        inbox = state_dir / rel
        if inbox.exists():
            shutil.rmtree(inbox)
...
def run_campaign(
    ...
    *,
    ...
    if campaign_pack.get("schema") != "rsi_real_campaign_pack_v1":
    ...
    base_policy_action = int(campaign_pack.get("base_policy_action", 3) or 3)
    if out_dir.exists():
    ...
    for epoch_idx in range(1, n_epochs + 1):
        ...
        if enable_family_generalizer and epoch_idx in insertion_epochs:
            propose_next_family(
                manifest=manifest,
        ...
        if enable_macro_miner and epoch_idx > 1 and macro_miner_top_k > 0:
            ...
            if run_miner:
                ...
                for idx in range(start_idx, epoch_idx):
                    trace_path = epochs_dir / f"epoch_{idx}" / "traces" / "trace_heldout_v1.jsonl"
        ...
        if enable_policy_synthesizer and epoch_idx > 1:
            run_synth = True
            if isinstance(policy_synth_epochs, list) and epoch_idx not in policy_synth_epochs:
                run_synth = False
            if run_synth:
                prev_diag = epochs_dir / f"epoch_{epoch_idx-1}" / "diagnostics"
