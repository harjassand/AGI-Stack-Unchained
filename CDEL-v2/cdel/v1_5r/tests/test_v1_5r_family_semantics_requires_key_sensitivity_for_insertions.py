...
def _make_family(*, salt: str, goal_x: int) -> dict:
    fam = {
...
def test_v1_5r_family_semantics_requires_key_sensitivity_for_insertions(tmp_path: Path) -> None:
    meta = meta_identities()
    camp_dir = tmp_path / "campaign"
    fam_dir = camp_dir / "families"
    fam_dir.mkdir(parents=True, exist_ok=True)

    core_family = _make_family(salt="core-001", goal_x=6)
    core_hash = hash_json(core_family)
    write_canon_json(fam_dir / f"{core_hash.split(':',1)[1]}.json", core_family)

    candidate = _make_family(salt="candidate-const", goal_x=7)
    cand_hash = hash_json(candidate)
    write_canon_json(fam_dir / f"{cand_hash.split(':',1)[1]}.json", candidate)
    ...
    pack = {
        ...
        "family_proposals_by_epoch": {"1": [f"families/{cand_hash.split(':',1)[1]}.json"]},
    ...
    key_check = checks.get("key_sensitive", {})
    assert key_check.get("ok") is False
    reasons = key_check.get("reason_codes", [])
    assert "FAMILY_NOT_KEY_SENSITIVE" in reasons
