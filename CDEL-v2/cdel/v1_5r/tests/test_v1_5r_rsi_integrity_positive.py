...
def test_v1_5r_rsi_integrity_positive(tmp_path: Path) -> None:
    ...
    receipts = list(out_dir.glob("epochs/epoch_*/diagnostics/rsi_integrity_receipt_v1.json"))
    assert len(receipts) == 1
    rsi_receipts = list(out_dir.glob("epochs/epoch_*/diagnostics/rsi_ignition_receipt_v1.json"))
    assert len(rsi_receipts) >= 1
