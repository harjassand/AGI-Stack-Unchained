    ...
    for epoch_dir in epoch_dirs:
        ...
        if integrity_receipt_path.exists():
            ...
            if canon_bytes(on_disk) != canon_bytes(integrity_result.integrity_receipt):
                # Legacy receipts may omit x-meta; allow strict match after stripping it.
                stripped_disk = dict(on_disk)
                stripped_disk.pop("x-meta", None)
                stripped_computed = dict(integrity_result.integrity_receipt)
                stripped_computed.pop("x-meta", None)
                if canon_bytes(stripped_disk) != canon_bytes(stripped_computed):
