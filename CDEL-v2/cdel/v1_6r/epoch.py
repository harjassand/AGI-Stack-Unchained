...
from .sr_cegar.gates import learnability_eval, novelty_pass
...
from .witness_constants import WITNESS_FAILURE_MODES_V1, WITNESS_REPLAY_KEY_DOMAIN_V1

ONTO_EVAL_V2_STAGE = "ONTO_EVAL_V2"
...
def run_epoch(
    *,
    ...
    macro_trace_paths: list[Path] | None = None,
) -> None:
        ...
        if fam_hash:
            ...
            if fam_path.exists():
                try:
                    ...
                    continue
    frontier_family_objs = list(family_objs)
    ...
    instance_witness_lookup: dict[str, dict[str, Any]] = {}
    current_idx = _epoch_index(epoch_id)
    for epoch_dir in _list_epoch_dirs(state_dir, out_dir):
        ep_name = epoch_dir.name
        if ep_name == epoch_id:
            continue
        ep_idx = _epoch_index(ep_name)
        if current_idx is not None and ep_idx is not None and ep_idx >= current_idx:
            continue
        if current_idx is None and ep_name >= epoch_id:
            continue
        ...
        for h in sorted(hashes):
            if h in instance_witness_lookup:
                continue
            witness_path = diag_dir / "instance_witnesses_v1" / f"{h.split(':', 1)[1]}.json"
            if witness_path.exists():
                instance_witness_lookup[h] = load_canon_json(witness_path)
    ...
    def _theta0_from_params(params_schema: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for param in params_schema:
            name = param.get("name")
            if not isinstance(name, str):
                raise ValueError("missing param name for theta0")
            ptype = param.get("type")
            min_val = param.get("min")
            if ptype == "int":
                if not isinstance(min_val, int):
                    raise ValueError("missing int min for theta0")
                theta0[name] = int(min_val)
            elif ptype == "fixed":
                if not isinstance(min_val, str):
                    raise ValueError("missing fixed min for theta0")
                theta0[name] = min_val
            else:
                raise ValueError("unknown param type for theta0")
        return theta0
    ...
    def _contains_keyed_op(node: Any) -> bool:
        if isinstance(node, dict):
            op = node.get("op")
            if op in {
                "KEYED_RAND_INT_V1",
                "KEYED_RAND_CHOICE_V1",
                "KEYED_RAND_STR_V1",
                "GRIDWORLD_BUILD_V1",
                "LINEWORLD_BUILD_V1",
                "EDITWORLD_BUILD_V1",
            ...
            for value in node.values():
                if _contains_keyed_op(value):
                    return True
        elif isinstance(node, list):
            return any(_contains_keyed_op(item) for item in node)
        return False

    gate_failures: list[dict[str, Any]] = []

    def _gate_failure_mode_from_suite_row(suite_row: dict[str, Any]) -> str:
        if not isinstance(suite_row, dict):
            return "INVALID_SUITE_ROW"
        ...
        if env_kind == "editworld-v1":
            ...
            max_goal_len = int(editworld.get("max_goal_len", 0)) if isinstance(editworld.get("max_goal_len"), int) else 0
            if suite_row.get("vocab_id") != vocab_id:
                return "INVALID_SUITE_ROW"
            ...
            if not isinstance(start_text, str) or not isinstance(goal_text, str):
                return "INVALID_SUITE_ROW"
            if not isinstance(start_cursor, int) or not isinstance(slip_ppm, int) or not isinstance(obs_window, int):
                return "INVALID_SUITE_ROW"
        ...
        if env_kind == "lineworld-v1":
            ...
            if not isinstance(length, int) or not isinstance(start, int) or not isinstance(goal, int):
                return "INVALID_SUITE_ROW"
            if length < 0 or start < 0 or goal < 0 or start > length or goal > length:
                return "INVALID_SUITE_ROW"
            return "UNKNOWN"
        if env_kind == "gridworld-v1":
            ...
            if not isinstance(start, dict) or not isinstance(goal, dict):
                return "INVALID_SUITE_ROW"
            if "x" not in start or "y" not in start or "x" not in goal or "y" not in goal:
                return "INVALID_SUITE_ROW"
            return "UNKNOWN"
        return "UNKNOWN"

    def _gate_failures_from_eval(family_id: str, gate_eval: dict[str, Any]) -> list[dict[str, Any]]:
        failures: list[dict[str, Any]] = []
        instance_specs = gate_eval.get("instance_specs", [])
        theta_list = gate_eval.get("theta_gate_list", [])
        results = gate_eval.get("results", [])
        if not isinstance(instance_specs, list):
            return failures
        success_by_theta: dict[int, bool] = {}
        if isinstance(results, list):
            for res in results:
                if not isinstance(res, dict):
                    continue
                idx = res.get("theta_index")
                if not isinstance(idx, int):
                    continue
                success_bit = int(res.get("success_bit", 0))
                if idx not in success_by_theta:
                    success_by_theta[idx] = True
                if success_bit == 0:
                    success_by_theta[idx] = False
        mark_all_failed = not results
        for idx, inst in enumerate(instance_specs):
            if not isinstance(inst, dict):
                continue
            if not mark_all_failed and success_by_theta.get(idx, True):
                continue
            payload = inst.get("payload") if isinstance(inst.get("payload"), dict) else {}
            suite_row = payload.get("suite_row") if isinstance(payload, dict) else None
            failure_mode = _gate_failure_mode_from_suite_row(suite_row if isinstance(suite_row, dict) else {})
            inst_hash = inst.get("inst_hash")
            if not isinstance(inst_hash, str):
                inst_hash = hash_json(inst)
            theta = theta_list[idx] if isinstance(theta_list, list) and idx < len(theta_list) else {}
            failures.append(
                {
                    "family_id": family_id,
                    "theta": theta,
                    "inst_hash": inst_hash,
                    "failure_kind": failure_mode,
                    "trace_events": [],
                    "instance_spec": inst,
                    "instance_kind": "gate",
                    "work_delta": {"env_steps_total": 0, "bytes_hashed_total": 0, "verifier_gas_total": 0},
                }
            )
        return failures
    for candidate in load_family_proposals(state_dir):
        validate_family_relaxed(candidate)
        is_witness_cond = candidate.get("x-provenance") == "witness_family_generalizer_v2"
        parent_hash = candidate.get("x-parent_witness_hash")
        parent_witness = instance_witness_lookup.get(parent_hash) if isinstance(parent_hash, str) else None
        parent_ok = True
        if is_witness_cond:
            parent_ok = isinstance(parent_hash, str) and (
                parent_hash in witness_hashes or parent_hash in instance_witness_lookup
            )
        ...
        witness_checks: dict[str, Any] | None = None
        keyed_ok = True
        lag_ok = True
        replay_ok = True
        mutation_ok = True
        if isinstance(checks, dict):
            ...
            if is_witness_cond:
                keyed_ok = _contains_keyed_op(candidate.get("instantiator"))
                lag_ok = False
                replay_ok = False
                mutation_ok = False
                ...
                if not isinstance(parent_witness, dict):
                    lag_reasons.append("FAMILY_MISSING_PARENT_WITNESS")
                    replay_reasons.append("FAMILY_MISSING_PARENT_WITNESS")
                    mutation_reasons.append("FAMILY_MISSING_PARENT_WITNESS")
                else:
                    ...
                    if parent_idx is not None and current_idx is not None:
                        lag_ok = parent_idx < current_idx
                    elif isinstance(parent_epoch, str):
                        lag_ok = parent_epoch < epoch_id
                    if not lag_ok:
                        lag_reasons.append("WITNESS_LAG_VIOLATION")
                    try:
                        replay_key = hashlib.sha256(
                            WITNESS_REPLAY_KEY_DOMAIN_V1.encode("utf-8") + _parse_prefixed_hash(parent_hash)
                        ).digest()
                    except Exception:
                        replay_key = b""
                    expected_hash = parent_witness.get("suite_row_hash")
                    replay_hash = _suite_row_hash_for_key(candidate, replay_key) if replay_key else None
                    replay_ok = isinstance(expected_hash, str) and replay_hash == expected_hash
                    if not replay_ok:
                        replay_reasons.append("FAMILY_WITNESS_REPLAY_MISMATCH")
                    probe_keys = constants.get("family_semantics", {})
                    key_a = probe_keys.get("probe_key_a")
                    key_b = probe_keys.get("probe_key_b")
                    hash_a = None
                    hash_b = None
                    if isinstance(key_a, str):
                        try:
                            hash_a = _suite_row_hash_for_key(candidate, _parse_prefixed_hash(key_a))
                        except Exception:
                            hash_a = None
                    if isinstance(key_b, str):
                        try:
                            hash_b = _suite_row_hash_for_key(candidate, _parse_prefixed_hash(key_b))
                        except Exception:
                            hash_b = None
                    mutation_ok = (
                        replay_hash is not None
                        and hash_a is not None
                        and hash_b is not None
                        and hash_a != replay_hash
                        and hash_b != replay_hash
                    )
                    if not mutation_ok:
                        mutation_reasons.append("FAMILY_WITNESS_NOT_MUTATING")
                witness_checks = {
                    "witness_keyed_op": {
                        "ok": bool(keyed_ok),
                        "reason_codes": [] if keyed_ok else ["FAMILY_NOT_KEYED"],
                    },
                    "witness_lag_ok": {"ok": bool(lag_ok), "reason_codes": lag_reasons},
                    "witness_replay_ok": {"ok": bool(replay_ok), "reason_codes": replay_reasons},
                    "witness_mutation_ok": {"ok": bool(mutation_ok), "reason_codes": mutation_reasons},
                }
        semantics_report["checks"] = checks
        if witness_checks:
            semantics_report["x-witness_checks"] = witness_checks
        ...
        extra_ok = True
        if is_witness_cond:
            extra_ok = bool(keyed_ok and lag_ok and replay_ok and mutation_ok)
        if not (parent_ok and key_sensitive_ok and fingerprint_ok and signature_ok and extra_ok):
            continue
        nov_ok, _min_dist = novelty_pass(candidate, families)
        learn_ok, gate_eval = learnability_eval(
            candidate,
            epoch_id=epoch_id,
        ...
        if isinstance(gate_eval, dict):
            gate_failures.extend(_gate_failures_from_eval(candidate.get("family_id", ""), gate_eval))
    ...
    def _is_witness_family(family_obj: dict[str, Any]) -> bool:
        if not isinstance(family_obj, dict):
            return False
        if isinstance(family_obj.get("x-parent_witness_hash"), str):
            return True
        if family_obj.get("x-provenance") == "witness_family_generalizer_v2":
            return True
        return False

    def _pressure_families(frontier_families: list[dict[str, Any]]) -> list[dict[str, Any]]:
        if not frontier_families:
            return []
        non_witness = [fam for fam in frontier_families if not _is_witness_family(fam)]
        if not non_witness:
            return list(frontier_families)
        non_witness_sorted = sorted(non_witness, key=lambda fam: str(fam.get("family_id", "")))
        out: list[dict[str, Any]] = []
        for fam in frontier_families:
            if not _is_witness_family(fam):
                out.append(fam)
                continue
            fam_id = str(fam.get("family_id", ""))
            digest = hashlib.sha256(fam_id.encode("utf-8")).digest()
            idx = int.from_bytes(digest[:4], "little") % len(non_witness_sorted)
            out.append(non_witness_sorted[idx])
        return out

    pressure_pack = build_pressure_pack(
        frontier_hash=frontier_hash,
        families=_pressure_families(frontier_family_objs),
    ...
    worst_anchor, anchor_failures, _, anchor_specs = _eval_pack(
        anchor_pack, instance_kind="anchor", record_traces=True, record_specs=True
    )
    worst_pressure, pressure_failures, _, pressure_specs = _eval_pack(
        pressure_pack, instance_kind="pressure", record_traces=True, record_specs=True
    )
    ...
    budget_steps_per_instance = int(constants.get("sr", {}).get("budget_env_steps_per_instance", 0) or 0)
    if budget_steps_per_instance > 0:
        n_anchor_instances = len(anchor_pack.get("families", [])) * n_anchor
        n_pressure_instances = len(pressure_pack.get("families", [])) * n_pressure
        budget_env_steps_anchor = int(n_anchor_instances) * budget_steps_per_instance
        budget_env_steps_pressure = int(n_pressure_instances) * budget_steps_per_instance
    else:
    ...
    # Instance witnesses (all failed instances)
    failures = anchor_failures + pressure_failures + gate_failures
    instance_witness_dir = diagnostics_dir / "instance_witnesses_v1"
    _ensure_dir(instance_witness_dir)
    witness_index_by_env: dict[str, dict[str, list[str]]] = {}

    def _trace_excerpt(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        if len(actions) <= 32:
            return actions
        return actions[:16] + actions[-16:]

    def _failure_mode_from_kind(failure_kind: str | None, env_kind: str, trace_events: list[dict[str, Any]]) -> str:
        if isinstance(failure_kind, str) and failure_kind in WITNESS_FAILURE_MODES_V1:
            return failure_kind
        if failure_kind == "INVALID_SUITE_ROW":
            return "INVALID_SUITE_ROW"
        if failure_kind == "VOCAB_TOKEN_INVALID":
            return "VOCAB_TOKEN_INVALID"
        if failure_kind == "GOAL_NOT_REACHED":
            if env_kind == "editworld-v1":
                noop_only = True
                for ev in trace_events:
                    ...
                    if name != "NOOP":
                        noop_only = False
                        break
                if noop_only and trace_events:
                    return "NO_PROGRESS_STALL"
            return "TIMEOUT_MAX_STEPS"
        return "UNKNOWN"

    for failure in failures:
        instance_spec = failure.get("instance_spec")
        payload = instance_spec.get("payload") if isinstance(instance_spec, dict) else None
        suite_row = payload.get("suite_row") if isinstance(payload, dict) else None
        if not isinstance(suite_row, dict):
            suite_row = {}
        ...
        instance_kind = failure.get("instance_kind")
        if instance_kind not in {"anchor", "pressure", "gate"}:
            instance_kind = "anchor"
        trace_events = failure.get("trace_events", [])
        if not isinstance(trace_events, list):
            trace_events = []
        trace_hash = hash_json(trace_events)
        trace_excerpt = _trace_excerpt(trace_events)
        suite_row_hash = sha256_prefixed(canon_bytes(suite_row))
        inst_hash = failure.get("inst_hash") or (instance_spec.get("inst_hash") if isinstance(instance_spec, dict) else None)
        if not isinstance(inst_hash, str):
            inst_hash = hash_json(instance_spec or {"suite_row": suite_row})
        failure_mode = _failure_mode_from_kind(failure.get("failure_kind"), env_kind, trace_events)
        work_delta = failure.get("work_delta")
        if not isinstance(work_delta, dict):
            work_delta = {}
        workvec = {
            "env_steps_total": int(work_delta.get("env_steps_total", 0)),
            "bytes_hashed_total": int(work_delta.get("bytes_hashed_total", 0)),
            "verifier_gas_total": int(work_delta.get("verifier_gas_total", 0)),
        }
        witness_payload = {
        ...
        witness_payload["x-meta"] = meta
        witness_hash = hash_json(witness_payload)
        write_canon_json(instance_witness_dir / f"{witness_hash.split(':', 1)[1]}.json", witness_payload)
        env_bucket = witness_index_by_env.setdefault(env_kind, {"anchor": [], "pressure": [], "gate": []})
        env_bucket.setdefault(instance_kind, [])
        env_bucket[instance_kind].append(witness_hash)

    for env_bucket in witness_index_by_env.values():
        for kind in ("anchor", "pressure", "gate"):
            env_bucket[kind] = sorted(set(env_bucket.get(kind, [])))

    witness_index_payload = {
        "schema": "instance_witness_index_v1",
        "schema_version": 1,
        "epoch_id": epoch_id,
        "witnesses_by_env_kind": witness_index_by_env,
    }
    witness_index_payload["x-meta"] = meta
    write_canon_json(diagnostics_dir / "instance_witness_index_v1.json", witness_index_payload)
    ...
    macro_trace_events = trace_events_hold
    macro_instance_specs = instance_specs_hold
    macro_trace_hashes: list[str] = []
    if macro_trace_paths:
        window_events: list[dict[str, Any]] = []
        window_specs: dict[str, Any] = {}
        for path in macro_trace_paths:
            if not path.exists():
                continue
            macro_trace_hashes.append(_hash_file(path))
            window_events.extend(load_trace_jsonl(str(path)))
            diag_path = path.parent.parent / "diagnostics"
            specs_path = diag_path / "instance_specs_v1.json"
            if specs_path.exists():
                specs_payload = load_canon_json(specs_path)
                instances = specs_payload.get("instances") if isinstance(specs_payload, dict) else None
                if isinstance(instances, dict):
                    for inst_hash, spec in instances.items():
                        if inst_hash in window_specs and window_specs[inst_hash] != spec:
                            raise ValueError("macro_window_instance_spec_collision")
                        window_specs[inst_hash] = spec
        if window_events:
            macro_trace_events = window_events
        if window_specs:
            macro_instance_specs = window_specs
    ...
    if admitted_family and next_frontier is not None:
        ...
        worst_pressure_next, _, _, _ = _eval_pack(
            pressure_pack_next,
            instance_kind="pressure",
            record_traces=False,
            record_specs=False,
            record_work=False,
            family_lookup=candidate_family_by_id,
        )
        ...
        _eval_pack(anchor_pack, instance_kind="anchor", record_traces=False, record_specs=False, family_lookup=family_by_id)
        _eval_pack(
            pressure_pack_next,
            instance_kind="pressure",
            record_traces=False,
    ...
    # Mechanism patch candidates (legacy dominance path)
    if not enable_mech_patch_searcher:
        for patch in load_mech_patch_proposals(state_dir):
            ...
            worst_anchor_next, _, _, _ = _eval_pack(
                ...
                record_specs=False,
                record_work=False,
                base_mech_override=candidate_mech,
            )
            worst_pressure_next, _, _, _ = _eval_pack(
                pressure_pack,
                instance_kind="pressure",
    ...
    cmech_selected_patch: dict[str, Any] | None = None
    cmech_selected_patch_id: str | None = None
    cmech_selected_cert: dict[str, Any] | None = None
    cmech_selected_cert_hash: str | None = None
    cmech_selected_patch_hash: str | None = None
    cmech_benchmark_hash: str | None = None

    if enable_mech_patch_searcher:
        mech_benchmark_path = state_dir / "current" / "mech_benchmark_pack_v1.json"
        mech_benchmark_pack = load_canon_json(mech_benchmark_path) if mech_benchmark_path.exists() else None
        mech_benchmark_hash = _hash_file(mech_benchmark_path) if mech_benchmark_path.exists() else None
        write_canon_json(diagnostics_dir / "mech_patch_base_mech_v1.json", base_mech_obj)
        ...
        base_patch_set_path = state_dir / "current" / "mech_patch_active_set_v1.json"
        base_patch_set_hash = _hash_file(base_patch_set_path) if base_patch_set_path.exists() else hash_json({})
        mech_patch_decisions: list[dict[str, Any]] = []
        mech_patch_pass: list[tuple[tuple[int, int, int, int], str, dict[str, Any], dict[str, Any], str]] = []
        cert_dir = diagnostics_dir / "mech_patch_eval_certs"

        for patch in load_mech_patch_proposals(state_dir):
            patch_id = patch.get("patch_id") or hash_json(patch)
            if not isinstance(patch_id, str):
                mech_patch_decisions.append(
                    {"patch_id": "UNKNOWN", "decision": "FAIL", "reason_codes": ["MECH_PATCH_ID_MISSING"]}
                )
                continue
            if current_state_head_hash and patch.get("base_state_hash") != current_state_head_hash:
                mech_patch_decisions.append(
                    {"patch_id": patch_id, "decision": "FAIL", "reason_codes": ["MECH_PATCH_BASE_STATE_MISMATCH"]}
                )
                continue
            if mech_benchmark_pack is None or mech_benchmark_hash is None:
                mech_patch_decisions.append(
                    {"patch_id": patch_id, "decision": "FAIL", "reason_codes": ["BENCHMARK_PACK_MISSING"]}
                )
                continue
            try:
                cert, case_deltas = compute_mech_patch_eval_cert(
                    epoch_id=epoch_id,
                    patch=patch,
                    base_mech=base_mech_obj,
                    benchmark_pack=mech_benchmark_pack,
                    base_patch_set_hash=base_patch_set_hash,
                    benchmark_pack_hash=mech_benchmark_hash,
                )
            except Exception:
                mech_patch_decisions.append(
                    {"patch_id": patch_id, "decision": "FAIL", "reason_codes": ["MECH_PATCH_EVAL_ERROR"]}
                )
                continue

            cert["x-meta"] = meta
            cert_dir.mkdir(parents=True, exist_ok=True)
            cert_path = cert_dir / f"{patch_id.split(':', 1)[1]}.mech_patch_eval_cert_v1.json"
            write_canon_json(cert_path, cert)
            cert_hash = _hash_file(cert_path)
            if cert.get("overall", {}).get("pass"):
                mech_patch_decisions.append({"patch_id": patch_id, "decision": "PASS", "reason_codes": []})
                total_delta = (
                    sum(delta[0] for delta in case_deltas),
                    sum(delta[1] for delta in case_deltas),
                    sum(delta[2] for delta in case_deltas),
                    sum(delta[3] for delta in case_deltas),
                )
                mech_patch_pass.append((total_delta, patch_id, patch, cert, cert_hash))
            else:
                reason_codes = cert.get("overall", {}).get("reason_codes", [])
                mech_patch_decisions.append(
                    {"patch_id": patch_id, "decision": "FAIL", "reason_codes": reason_codes or ["MECH_PATCH_REJECT"]}
                )

        if mech_patch_decisions:
            mech_patch_report = {
                "schema": "mech_patch_admission_report_v1",
                "schema_version": 1,
                "epoch_id": epoch_id,
                "decisions": mech_patch_decisions,
            }
            mech_patch_report["x-meta"] = meta
            write_canon_json(diagnostics_dir / "mech_patch_admission_report_v1.json", mech_patch_report)
            admitted_ids = [
                d.get("patch_id")
                for d in mech_patch_decisions
                if d.get("decision") == "PASS" and isinstance(d.get("patch_id"), str)
            ]
            admit_receipt = {
                "schema": "mech_patch_admit_receipt_v1",
                "schema_version": 1,
                "epoch_id": epoch_id,
                "admitted_patch_ids": admitted_ids,
                "report_hash": hash_json(mech_patch_report),
            }
            admit_receipt["x-meta"] = meta
            write_canon_json(diagnostics_dir / "mech_patch_admit_receipt_v1.json", admit_receipt)

        if mech_patch_pass:
            best = None
            for item in mech_patch_pass:
                if best is None or item[0] > best[0] or (item[0] == best[0] and item[1] < best[1]):
                    best = item
            if best is not None:
                _delta, patch_id, patch, cert, cert_hash = best
                cert = dict(cert)
                overall = dict(cert.get("overall", {})) if isinstance(cert.get("overall"), dict) else {}
                overall["selected"] = True
                cert["overall"] = overall
                write_canon_json(diagnostics_dir / "mech_patch_eval_cert_v1.json", cert)
                candidate_dir = diagnostics_dir / "candidate_mech_patches_v2"
                candidate_dir.mkdir(parents=True, exist_ok=True)
                candidate_patch_path = candidate_dir / f"{patch_id.split(':', 1)[1]}.json"
                write_canon_json(candidate_patch_path, patch)
                cmech_selected_patch = patch
                cmech_selected_patch_id = patch_id
                cmech_selected_cert = cert
                cmech_selected_cert_hash = _hash_file(diagnostics_dir / "mech_patch_eval_cert_v1.json")
                cmech_selected_patch_hash = _hash_file(candidate_patch_path)
                cmech_benchmark_hash = mech_benchmark_hash
    ...
    if cmech_selected_patch and cmech_selected_patch_id and cmech_selected_cert_hash and cmech_benchmark_hash:
        patches_dir = state_dir / "current" / "mech_patches"
        patches_dir.mkdir(parents=True, exist_ok=True)
        state_patch_path = patches_dir / f"{cmech_selected_patch_id.split(':', 1)[1]}.json"
        write_canon_json(state_patch_path, cmech_selected_patch)
        base_mech_payload = load_canon_json(state_dir / "current" / "base_mech.json")
        updated_mech = _apply_mech_patch(base_mech_payload, cmech_selected_patch)
        write_canon_json(state_dir / "current" / "base_mech.json", updated_mech)
        active_set_path = state_dir / "current" / "mech_patch_active_set_v1.json"
        active_set = (
            load_canon_json(active_set_path)
            if active_set_path.exists()
            else {
                "schema": "mech_patch_active_set_v1",
                "schema_version": 1,
                "active_patch_ids": [],
                "ledger_head_hash": current_state_head_hash or "sha256:" + "0" * 64,
            }
        )
        active_ids = list(active_set.get("active_patch_ids", [])) if isinstance(active_set, dict) else []
        if cmech_selected_patch_id not in active_ids:
            active_ids.append(cmech_selected_patch_id)
        if isinstance(active_set, dict):
            active_set["active_patch_ids"] = sorted(set(active_ids))
            if not isinstance(active_set.get("ledger_head_hash"), str):
                active_set["ledger_head_hash"] = current_state_head_hash or "sha256:" + "0" * 64
            write_canon_json(active_set_path, active_set)

    if selected_candidate and selected_candidate.get("promotion_type") == "macro_admit":
        macro_active_set_path = state_dir / "current" / "macro_active_set_v1.json"
        if macro_active_set_path.exists():
            macro_active_set = load_canon_json(macro_active_set_path)
            active_ids = list(macro_active_set.get("active_macro_ids", [])) if isinstance(macro_active_set, dict) else []
            macro_defs_post = load_macro_defs(state_dir / "current" / "macros", allowed=active_ids)
            report = build_macro_cross_env_support_report(
                epoch_id=epoch_id,
                trace_events=macro_trace_events,
                macro_defs=macro_defs_post,
                macro_active_set_hash=_hash_file(macro_active_set_path),
                instance_specs=macro_instance_specs,
            )
            if macro_trace_hashes:
                report["x-trace_hashes"] = macro_trace_hashes
            report["x-meta"] = meta
            write_canon_json(diagnostics_dir / "macro_cross_env_support_report_v1.json", report)
    ...
    mech_active_path = state_dir / "current" / "mech_patch_active_set_v1.json"
    mech_active_hash = _hash_file(mech_active_path) if mech_active_path.exists() else hash_json({})
    pointer_hashes = {
        ...
        "meta_patch_set_hash": _hash_file(state_dir / "current" / "meta_patch_set_v1.json"),
        "mech_patch_active_set_hash": mech_active_hash,
    }
    state_entry = {
    ...
    if cmech_selected_patch_id and cmech_selected_cert_hash and cmech_benchmark_hash:
        state_entry["mech_patch_event"] = {
            "event_type": "MECH_PATCH_ACTIVATE_V2",
            "schema_version": 1,
            "patch_id": cmech_selected_patch_id,
            "patch_hash": cmech_selected_patch_hash,
            "base_state_hash": cmech_selected_patch.get("base_state_hash") if cmech_selected_patch else None,
            "benchmark_pack_hash": cmech_benchmark_hash,
            "eval_cert_hash": cmech_selected_cert_hash,
            "reason_code": "MECH_PATCH",
        }
    ...
    eval_budget_report = {
        ...
        "counts": {
            "n_anchor_instances": int(n_anchor_instances),
            "n_pressure_instances": int(n_pressure_instances),
            "n_gate_instances": 0,
        },
        "budgets": {
    ...
    for epoch_dir in epoch_dirs:
        ...
        macro_cross_env_path = diag_dir / "macro_cross_env_support_report_v1.json"
        if macro_cross_env_path.exists():
            report = load_canon_json(macro_cross_env_path)
            report_epoch = report.get("epoch_id")
            if not isinstance(report_epoch, str):
                report_epoch = epoch_dir.name
            macro_cross_env_reports[report_epoch] = report
            macro_cross_env_hashes[report_epoch] = _hash_file(macro_cross_env_path)
        ...
        mech_cert_dir = diag_dir / "mech_patch_eval_certs"
        if mech_cert_dir.exists():
            for path in sorted(mech_cert_dir.glob("*.mech_patch_eval_cert_v1.json")):
                cert = load_canon_json(path)
                patch_id = cert.get("patch_id")
                if isinstance(patch_id, str) and patch_id not in mech_patch_eval_certs:
                    mech_patch_eval_certs[patch_id] = cert
                    mech_patch_eval_cert_hashes[patch_id] = _hash_file(path)
    ...
    transfer_state_path = state_dir / "current" / "rsi_transfer_tracker_state_v1.json"
    prior_transfer_state = load_canon_json(transfer_state_path) if transfer_state_path.exists() else None
    families_lookup: dict[str, Any] = {}
    families_dir = state_dir / "current" / "families"
    ...
    transfer_epoch_artifacts = {
        ...
        "rsi_portfolio_receipt_hash": _hash_file(diagnostics_dir / "rsi_portfolio_receipt_v1.json")
        if (diagnostics_dir / "rsi_portfolio_receipt_v1.json").exists()
        else None,
        "barrier_ledger_entries": barrier_entries,
        "state_ledger_events": state_ledger_events,
        "family_semantics_reports": family_semantics_reports,
    ...
    transfer_result = update_rsi_transfer_tracker(
        ...
        strict=strict_transfer,
    )
    write_canon_json(transfer_state_path, transfer_result.state)
    write_canon_json(diagnostics_dir / "rsi_transfer_window_report_v1.json", transfer_result.window_report)
    if transfer_result.transfer_receipt is not None:
        write_canon_json(diagnostics_dir / "rsi_transfer_receipt_v1.json", transfer_result.transfer_receipt)
    ...
    receipt_artifacts = [
        ...
        {"name": "rsi_transfer_window_report_v1.json", "hash": _hash_file(diagnostics_dir / "rsi_transfer_window_report_v1.json")},
    ...
    if transfer_receipt_path.exists():
        receipt_artifacts.append(
            {"name": "rsi_transfer_receipt_v1.json", "hash": _hash_file(transfer_receipt_path)}
        )
    ...
    macro_cross_env_path = diagnostics_dir / "macro_cross_env_support_report_v1.json"
    if macro_cross_env_path.exists():
        receipt_artifacts.append(
            {"name": "macro_cross_env_support_report_v1.json", "hash": _hash_file(macro_cross_env_path)}
        )
    mech_eval_cert_path = diagnostics_dir / "mech_patch_eval_cert_v1.json"
    if mech_eval_cert_path.exists():
        receipt_artifacts.append(
            {"name": "mech_patch_eval_cert_v1.json", "hash": _hash_file(mech_eval_cert_path)}
        )
