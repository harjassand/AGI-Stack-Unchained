...
def _fallback_parent_witness(state_dir: Path) -> str | None:
    epochs_dir = state_dir / "epochs"
    if not epochs_dir.exists():
        return None
    candidates: list[tuple[int, str]] = []
    for epoch_dir in sorted(epochs_dir.glob("epoch_*")):
        epoch_idx = _epoch_index(epoch_dir.name)
        if epoch_idx is None:
            continue
        index_path = epoch_dir / "diagnostics" / "instance_witness_index_v1.json"
        ...
        for env_bucket in by_env.values():
            ...
            for kind in ("anchor", "pressure", "gate"):
                entries = env_bucket.get(kind)
                if isinstance(entries, list):
                    hashes.extend([h for h in entries if isinstance(h, str)])
        if not hashes:
            continue
        hashes = sorted(set(hashes))
        candidates.append((epoch_idx, hashes[-1]))
    if not candidates:
        return None
...
def propose_next_family(
    *,
    ...
    parent_hash = witness_hashes[-1] if witness_hashes else _fallback_parent_witness(state_dir)
    ...
    suite_steps = max(6, int(bounds_steps))
    if suite_steps % 2 == 1:
        suite_steps += 1
    bounds_steps = max(int(bounds_steps), int(suite_steps))
    goal_max = 2
