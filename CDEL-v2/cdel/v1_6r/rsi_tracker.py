...
def update_rsi_tracker(
    *,
    ...
    if isinstance(open_ins, dict) and open_ins.get("started"):
        ...
        evidence = open_ins.get("epoch_evidence")
        evidence_len = len(evidence) if isinstance(evidence, list) else 0
        if worst_anchor >= tau_anchor and worst_heldout >= tau_heldout and has_selection and evidence_len >= 2:
            recovered = True
    ...
    if window_start_epoch:
        ...
        insertion_epochs = {
            ins.get("insertion_epoch") or ins.get("start_epoch")
            for ins in state.get("recent_insertions", [])
            if isinstance(ins, dict)
        }
        insertion_epochs = {e for e in insertion_epochs if isinstance(e, str)}
        if insertion_epochs:
            state["rho_series"] = [
                entry for entry in state.get("rho_series", []) if entry.get("epoch_id") in insertion_epochs
            ]
...
def build_rsi_ontology_receipt_v2(
    *,
    run_id: str,
    final_epoch: str,
    ontology_root: Path,
    ...
    admitted = sum(1 for entry in entries if entry.get("event") == "ADMIT")
    activated = sum(1 for entry in entries if entry.get("event") == "ACTIVATE")
    evicted = sum(1 for entry in entries if entry.get("event") == "EVICT")

    reports_dir = ontology_root / "reports"
    latest_report = None
    latest_idx = None
    ...
    if not isinstance(latest_report, dict):
        raise CanonError("missing ontology_eval_report_v2")

    dl_block = latest_report.get("dl") if isinstance(latest_report.get("dl"), dict) else {}
    dl_gain_bits = int(dl_block.get("dl_gain_bits", 0))
    support = int(dl_block.get("support_families_improved", 0))
    corpus = latest_report.get("corpus") if isinstance(latest_report.get("corpus"), dict) else {}
    ...
    verdict = "VALID"
    if admitted < 1 or activated < 1:
        verdict = "INVALID"
    if dl_gain_bits < min_gain or support < min_support:
        verdict = "INVALID"

    receipt = {
        "schema": "rsi_ontology_receipt_v2",
        "schema_version": 2,
        "run_id": run_id,
        "final_epoch": final_epoch,
        "ontology_events": {"admitted": admitted, "activated": activated, "evicted": evicted},
        "latest_eval": {
            "ontology_id": latest_report.get("new_ontology_id"),
            "dl_gain_bits": dl_gain_bits,
            "support_families_improved": support,
