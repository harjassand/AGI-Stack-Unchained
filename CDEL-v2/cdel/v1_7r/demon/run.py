...
from ..canon import CanonError, canon_bytes, hash_json, load_canon_json, write_canon_json, write_jsonl_line
from ..constants import meta_identities, require_constants
from ..demon.inbox import load_proposals_dir
from ..demon.trace import build_trace_event_v2
from ..ontology_v3.context_kernel import build_null_ctx_key, ctx_hash
from ..ontology_v3.dl_metric import context_hash_for_event
from ..ontology_v3.eval import evaluate_epoch as eval_ontology_v3
from ..ontology_v3.io import ensure_ontology_dirs, load_def_by_ontology_id, load_snapshot_by_id
from ..macros_v2.eval import evaluate_epoch as eval_macros_v2, maybe_evict as maybe_evict_macros
...
def run_campaign(*, campaign_pack: Path, out_dir: Path, mode: str, strict: bool) -> None:
    ...
    if pack.get("schema") != "rsi_real_demon_campaign_pack_v3":
        raise CanonError("campaign_pack schema mismatch")
    if int(pack.get("schema_version", 0)) != 3:
    ...
    if not isinstance(onto_dir, str) or not isinstance(macro_dir, str):
