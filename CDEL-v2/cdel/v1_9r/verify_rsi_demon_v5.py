"""Verifier for RSI demon v5 autonomy runs."""
...
from ..v1_8r.demon.tracker import build_receipt
from ..v1_8r.metabolism_v1.io import compute_patch_id, ensure_metabolism_dirs, load_patch_def
from ..v1_8r.metabolism_v1.ledger import load_ledger_entries as load_metabolism_ledger
...
def _verify_campaign_pack(state_dir: Path) -> dict[str, Any]:
    ...
    if pinned.get("schema") != "rsi_real_demon_campaign_pack_v5" or int(pinned.get("schema_version", 0)) != 5:
        _fail("AUTONOMY_PACK_MISMATCH")

    source_path = _repo_root() / "campaigns" / "rsi_real_demon_v5_autonomy" / "rsi_real_demon_campaign_pack_v5.json"
    if not source_path.exists():
        _fail("AUTONOMY_PACK_MISMATCH")
    source = load_canon_json(source_path)
    if source.get("schema") != "rsi_real_demon_campaign_pack_v5" or int(source.get("schema_version", 0)) != 5:
        _fail("AUTONOMY_PACK_MISMATCH")
    ...
    if not isinstance(proposals, dict):
        _fail("AUTONOMY_PACK_MISMATCH")
    if proposals.get("metabolism_v1_dir") != "__AUTONOMY_RUNDIR_V1__":
        _fail("AUTONOMY_PACK_MISMATCH")
    if not isinstance(autonomy, dict) or not autonomy.get("enabled"):
        _fail("AUTONOMY_PACK_MISMATCH")
    if not isinstance(metabolism, dict):
        _fail("AUTONOMY_PACK_MISMATCH")
    if metabolism.get("algorithm") != "autopatch_enum_v1":
        _fail("AUTONOMY_PACK_MISMATCH")
    if metabolism.get("manifest_rundir_path") != "autonomy/metabolism_v1/autonomy_manifest_v1.json":
        _fail("AUTONOMY_PACK_MISMATCH")
...
def _validate_manifest(manifest: dict[str, Any]) -> None:
    ...
    if set(manifest.keys()) != required_keys:
        _fail("AUTONOMY_MANIFEST_INVALID")
    if manifest.get("schema") != "autonomy_manifest_v1" or int(manifest.get("schema_version", 0)) != 1:
        _fail("AUTONOMY_MANIFEST_INVALID")
    if manifest.get("autonomy_kind") != "metabolism_autonomy_v1":
        _fail("AUTONOMY_MANIFEST_INVALID")
    if manifest.get("algorithm") != "autopatch_enum_v1":
        _fail("AUTONOMY_MANIFEST_INVALID")
    if manifest.get("output_subdir") != "autonomy/metabolism_v1/proposals":
        _fail("AUTONOMY_MANIFEST_INVALID")
    if not isinstance(manifest.get("patches"), list):
        _fail("AUTONOMY_MANIFEST_INVALID")
...
def _verify_autonomy(state_dir: Path, campaign_pack: dict[str, Any]) -> dict[str, Any]:
    manifest_path = state_dir / "autonomy" / "metabolism_v1" / "autonomy_manifest_v1.json"
    if not manifest_path.exists():
        _fail("AUTONOMY_MANIFEST_INVALID")
    manifest = load_canon_json(manifest_path)
    _validate_manifest(manifest)

    # Recompute enumerator output.
    ...
    if not isinstance(translation_path, str):
        _fail("AUTONOMY_PACK_MISMATCH")
    translation_inputs = load_translation_inputs(_repo_root() / "campaigns" / "rsi_real_demon_v5_autonomy" / translation_path)
    expected_manifest, expected_patch_defs = compute_expected(translation_inputs)

    if manifest.get("translation_inputs_hash") != expected_manifest.get("translation_inputs_hash"):
        _fail("AUTONOMY_INPUT_HASH_MISMATCH")
    if manifest.get("constants_hash") != expected_manifest.get("constants_hash"):
        _fail("AUTONOMY_INPUT_HASH_MISMATCH")

    if manifest.get("patches") != expected_manifest.get("patches"):
        _fail("AUTONOMY_ENUM_MISMATCH")

    # Proposals directory integrity.
    proposals_dir = state_dir / "autonomy" / "metabolism_v1" / "proposals"
    if not proposals_dir.exists():
        _fail("AUTONOMY_PROPOSALS_DIR_MISMATCH")
    ...
    # Ensure patch defs match expected.
...
def _verify_metabolism(state_dir: Path, constants: dict[str, Any], meta: dict[str, str], campaign_pack: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(translation_path, str):
        _fail("AUTONOMY_PACK_MISMATCH")
    translation_inputs = load_translation_inputs(_repo_root() / "campaigns" / "rsi_real_demon_v5_autonomy" / translation_path)
    translation_inputs_hash = sha256_prefixed(canon_bytes(translation_inputs))
    if report.get("translation_inputs_hash") != translation_inputs_hash:
        _fail("translation_inputs_hash mismatch")

    min_delta = int(constants.get("METAPATCH_V1_MIN_SHA256_CALL_DELTA", 1) or 1)
    eval_result = evaluate_translation(
        translation_inputs=translation_inputs,
        cache_capacity=capacity,
        min_sha256_delta=min_delta,
    )
...
def _verify_receipt(state_dir: Path) -> None:
    ...
    expected = build_receipt(state_dir=state_dir, run_id=receipt.get("run_id", ""), final_epoch=6)
    if receipt != expected:
...
def verify(state_dir: Path) -> None:
    ...
    _verify_autonomy(state_dir, campaign_pack)
    ...
    _verify_metabolism(state_dir, constants_meta, meta_meta, campaign_pack)
    _verify_receipt(state_dir)


def main() -> None:
    parser = argparse.ArgumentParser(description="Verify RSI demon v5 autonomy run")
