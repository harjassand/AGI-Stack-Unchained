"""Entry point for RSI demon v6 efficiency campaigns."""
...
def _validate_out_dir(out_dir: Path) -> None:
    ...
    if not entries:
        return
    if len(entries) == 1 and entries[0] == "autonomy":
        return
...
def _write_receipt_v6(*, state_dir: Path, final_epoch: int, last_eval: dict[str, Any]) -> Path:
    metabolism_root = state_dir / "current" / "metabolism_v1"
    ...
    out_path = state_dir / "epochs" / f"epoch_{final_epoch}" / "diagnostics" / "rsi_demon_receipt_v6.json"
...
def run_campaign(*, campaign_pack: Path, out_dir: Path, mode: str, strict: bool) -> None:
    ...
    if pack.get("schema") != "rsi_real_demon_campaign_pack_v6":
        raise CanonError("campaign_pack schema mismatch")
    if int(pack.get("schema_version", 0)) != 6:
    ...
    if metabolism_dir != "__AUTONOMY_RUNDIR_V2__":
        raise CanonError("campaign_pack metabolism_v1_dir invalid")
    ...
    if not isinstance(autonomy_cfg, dict) or not autonomy_cfg.get("enabled"):
    ...
    proposals_subdir = metabolism_cfg.get("proposals_rundir_subdir")
    if proposals_subdir != "autonomy/metabolism_v1/proposals":
        raise CanonError("campaign_pack autonomy proposals subdir mismatch")
    ...
    metabolism_proposals_dir = out_dir / proposals_subdir
    ...
    for epoch_idx in range(1, epochs + 1):
        ...
        if eval_metabolism_every and epoch_idx % eval_metabolism_every == 0:
            last_eval = _evaluate_metabolism_v2(
                state_dir=out_dir,
                epoch_id=f"epoch_{epoch_idx}",
    ...
    _write_receipt_v6(state_dir=out_dir, final_epoch=epochs, last_eval=last_eval)


def main() -> None:
    parser = argparse.ArgumentParser(description="Run RSI demon v6 efficiency campaign")
