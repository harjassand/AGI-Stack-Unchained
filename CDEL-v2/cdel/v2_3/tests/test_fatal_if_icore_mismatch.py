from __future__ import annotations

from pathlib import Path

import pytest

from cdel.v1_7r.canon import CanonError
from cdel.v2_3.verify_rsi_demon_v9 import verify

from .utils import build_valid_icore_receipt, write_minimal_attempt


def test_fatal_if_icore_mismatch(tmp_path: Path, repo_root: Path) -> None:
    receipt = build_valid_icore_receipt(repo_root)
    receipt["core_id_observed"] = "sha256:" + "1" * 64
    head = dict(receipt)
    head.pop("receipt_head_hash", None)
    from cdel.v1_7r.canon import canon_bytes, sha256_prefixed

    receipt["receipt_head_hash"] = sha256_prefixed(canon_bytes(head))

    attempt_dir = write_minimal_attempt(
        tmp_path,
        repo_root,
        include_receipt=True,
        receipt_override=receipt,
        touched_relpath="Extension-1/agi-orchestrator/orchestrator/proposer/csi_hotpath_v1.py",
    )

    with pytest.raises(CanonError) as excinfo:
        verify(attempt_dir)

    assert str(excinfo.value) == "IMMUTABLE_CORE_ATTESTATION_INVALID"
