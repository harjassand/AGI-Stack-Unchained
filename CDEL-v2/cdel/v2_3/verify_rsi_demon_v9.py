"""Verifier for RSI demon v9 CSI hardening attempts."""
...
from .immutable_core import load_lock, lock_fileset, validate_lock, validate_receipt
...
def _run_bench(tree_dir: Path, suite_path: Path, inputs_path: Path, run_id: str, attempt_id: str, tree_hash: str, out_path: Path) -> dict[str, Any]:
    ...
    result = subprocess.run(cmd, env=env, cwd=tree_dir, check=False, capture_output=True, text=True)
...
def verify(state_dir: Path) -> dict[str, Any]:
    ...
    receipt_path = run_root / "diagnostics" / "immutable_core_receipt_v1.json"
    ...
    locked_files = lock_fileset(lock)
    ...
    for entry in touched:
        ...
        if not isinstance(relpath, str):
            _fail("SCHEMA_INVALID")
        if relpath in locked_files:
            _fail("CSI_IMMUTABLE_CORE_TOUCH")
    ...
    try:
        ...
        raise CanonError("CSI_STATIC_SAFETY_FAIL") from exc
    ...
    try:
        ...
        raise CanonError("CSI_PATCH_APPLY_FAIL") from exc
...
def _write_receipt(state_dir: Path, receipt: dict[str, Any]) -> None:
    out = state_dir / "diagnostics" / "rsi_demon_receipt_v9.json"
    write_canon_json(out, receipt)


def main() -> None:
    parser = argparse.ArgumentParser(description="Verify RSI demon v9 CSI attempt")
