"""Verifier for RSI hardening v2.3 runs."""
...
from .verify_rsi_demon_v9 import verify as verify_attempt
...
def compute_run_id(pack_hash: str, constitution_hash: str) -> str:
    data = (pack_hash + constitution_hash + "hardening_v2_3").encode("utf-8")
...
def _load_pack_used(state_dir: Path) -> dict[str, Any]:
    ...
    if pack.get("schema") != "rsi_real_hardening_pack_v1":
...
def _load_ledger(state_dir: Path) -> list[dict[str, Any]]:
    ledger_path = state_dir / "ledger" / "hardening_ledger_v1.jsonl"
...
def _verify_ledger(entries: list[dict[str, Any]]) -> tuple[str, list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for entry in entries:
        if entry.get("schema") != "hardening_ledger_event_v1":
            _fail("SCHEMA_INVALID")
        ...
        head = expected
        seq += 1
        if entry.get("event") == "HARDENING_PATCH_ACCEPT_V1":
            accepts.append(entry)
        elif entry.get("event") == "HARDENING_PATCH_REJECT_V1":
            rejects.append(entry)
    return head, accepts, rejects
...
def _find_attempt_dir(state_dir: Path, patch_id: str) -> Path | None:
    ...
    for path in sorted(attempts_dir.glob("attempt_*")):
        ...
        if patch.get("patch_id") == patch_id:
            receipt_path = path / "diagnostics" / "rsi_demon_receipt_v9.json"
...
def verify(state_dir: Path) -> dict[str, Any]:
    ...
    if not isinstance(lock_rel, str):
        _fail("IMMUTABLE_CORE_LOCK_INVALID")
    ...
    try:
        ...
        raise CanonError("IMMUTABLE_CORE_LOCK_INVALID") from exc
    ...
    ledger_head_hash, accepts, rejects = _verify_ledger(entries)

    required_k = int(pack.get("K_required_patches", 0) or 0)
    if len(accepts) < required_k:
        _fail("MISSING_ARTIFACT")

    reject_reason = pack.get("require_rejection_reason")
    reject_min = int(pack.get("require_rejection_min", 0) or 0)
    if not isinstance(reject_reason, str) or reject_min <= 0:
        _fail("SCHEMA_INVALID")
    reject_seqs = []
    for entry in rejects:
        ...
        if not isinstance(payload, dict):
            _fail("SCHEMA_INVALID")
        if payload.get("reason") == reject_reason:
            reject_seqs.append(int(entry.get("seq", -1)))
    if len(reject_seqs) < reject_min:
        _fail("MISSING_ARTIFACT")

    require_accept_after = bool(pack.get("require_accept_after_reject", False))
    if require_accept_after:
        if not reject_seqs:
            _fail("MISSING_ARTIFACT")
        first_reject = min(reject_seqs)
        has_accept_after = any(int(entry.get("seq", -1)) > first_reject for entry in accepts)
        if not has_accept_after:
            _fail("MISSING_ARTIFACT")
    ...
    receipt = {
        "schema": "rsi_hardening_receipt_v1",
        "run_id": run_id,
        "pack_hash": pack_hash,
...
def _write_receipt(state_dir: Path, receipt: dict[str, Any]) -> None:
    out = state_dir / "diagnostics" / "rsi_hardening_receipt_v1.json"
    write_canon_json(out, receipt)


def main() -> None:
    parser = argparse.ArgumentParser(description="Verify RSI hardening v2.3 run")
