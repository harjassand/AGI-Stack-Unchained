...
def test_v3_1_subswarm_duplicate_join_fatal(tmp_path: Path, repo_root: Path) -> None:
    ...
    join_event = next(e for e in events if e.get("event_type") == "SUBSWARM_JOIN_ACCEPT")
    end_index = next(i for i, e in enumerate(events) if e.get("event_type") == "SWARM_END")
    events.insert(end_index, dict(join_event))
