...
def test_v3_2_barrier_bridge_stale_context_fatal(tmp_path, repo_root) -> None:
    ...
    barrier_entry = {
        ...
        "evidence": {
            ...
            "bridge_provenance": {
                ...
                "imported_artifacts_set_hash": import_info["imported_set_hash"],
                "staleness": {"is_stale": True, "reason": "STALE_CONTEXT"},
            },
        },
        "entry_hash": "__SELF__",
    }
    ...
    write_graph_report(run_root, ctx["run_id"], knowledge_edges=[])

    with pytest.raises(CanonError) as exc:
        verify(run_root)
    assert "STALE_CONTEXT_BARRIER_UPDATE" in str(exc.value)
