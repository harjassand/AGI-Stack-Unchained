...
def test_v3_2_bridge_icore_mismatch_fatal(tmp_path, repo_root) -> None:
    ctx = setup_base_run(tmp_path, repo_root, with_result=True)
    run_root = ctx["run_root"]
    wrong_icore = "sha256:" + "2" * 64
    offer_info = create_offer(run_root, ctx["run_id"], icore_id=ctx["icore_id"], task_id=ctx["task_id"], result_id=ctx["result_id"], verify_ref=ctx["verify_ref"], icore_override=wrong_icore)
    ...
    with pytest.raises(CanonError) as exc:
        verify(run_root)
    assert "BRIDGE_ICORE_MISMATCH" in str(exc.value)
