...
def test_v3_2_bridge_unverified_export_fatal(tmp_path, repo_root) -> None:
    ctx = setup_base_run(tmp_path, repo_root, with_result=True)
    run_root = ctx["run_root"]
    offer_info = create_offer(run_root, ctx["run_id"], icore_id=ctx["icore_id"], task_id=ctx["task_id"], result_id=ctx["result_id"], verify_ref=ctx["verify_ref"], verify_ref_override="sha256:" + "3" * 64)
    ...
    with pytest.raises(CanonError) as exc:
        verify(run_root)
    assert "BRIDGE_UNVERIFIED_EXPORT" in str(exc.value)
