"""Verifier for RSI swarm v3.2 runs (lateral bridge)."""
...
def _verify_node(
    ...
    # Swarm ledger
    swarm_path = state_dir / "ledger" / "swarm_ledger_v4.jsonl"
    ...
    if pack.get("schema") != "rsi_real_swarm_pack_v3" or pack.get("spec_version") != "v3_2":
        _fail("SCHEMA_INVALID")
...
def _write_receipt(state_dir: Path, receipt: dict[str, Any]) -> None:
    out_path = state_dir / "diagnostics" / "rsi_swarm_receipt_v4.json"
...
def _verify_bridge_imports(ctx: VerifyContext, offers: dict[str, dict[str, Any]], root_run_id: str) -> list[dict[str, str]]:
    ...
    for node in ctx.nodes:
        ...
        ledger_path = node_dir / "ledger" / "swarm_ledger_v4.jsonl"
        ...
        for event in import_accepts:
            ...
            if pub_events is None:
                ...
                pub_events = load_swarm_ledger(pub_dir / "ledger" / "swarm_ledger_v4.jsonl")
...
def _verify_bridge_barrier_entries(ctx: VerifyContext, offers: dict[str, dict[str, Any]]) -> None:
    for node in ctx.nodes:
        ...
        barrier_entries = load_barrier_ledger(barrier_path)
        swarm_events = load_swarm_ledger(node_dir / "ledger" / "swarm_ledger_v4.jsonl")
...
def _verify_swarm_graph_report(ctx: VerifyContext, root_run_id: str, knowledge_edges: list[dict[str, str]]) -> None:
    report_path = ctx.root_dir / "diagnostics" / "swarm_graph_report_v1.json"
...
def verify(state_dir: Path, *, allow_child: bool = False) -> dict[str, Any]:
    ...
    _verify_swarm_graph_report(ctx, receipt.get("run_id", ""), knowledge_edges)

    return receipt
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify RSI swarm v3.2 run")
