...
def test_v3_3_meta_policy_out_of_bounds_fatal(tmp_path, repo_root) -> None:
    ctx = setup_base_run(tmp_path, repo_root, max_epochs=1, with_result=True)
    ...
    update = build_policy_update(
        ...
        publisher=publisher,
        topics=["demo/lemmaA"],
        policy_delta={"bridge.subscriptions_remove": ["demo/lemmaA"]},
    )
    ...
    with pytest.raises(CanonError) as exc:
        verify(run_root)
    assert "META_POLICY_OUT_OF_BOUNDS" in str(exc.value)
