...
def _write_test_pack(tmp_path: Path, repo_root: Path) -> Path:
    ...
    dev_cfg = repo_root / "CDEL-v2" / "configs" / "sealed_env_dev.toml"
    ...
    pack = {
        ...
        "omega": {
            ...
            "self_improvement": {
                ...
                "proposal_systems": {
                    "autonomy_v2": True,
                    "recursive_ontology_v2_1": True,
                    "csi_v2_2": True,
                    "hardening_v2_3": True,
                },
                "promotion_policy": "PROMOTE_ONLY_IF_DEV_IMPROVES_V1",
                "dev_gate": {"sealed_config_path": str(dev_cfg), "min_delta_score_num": 1, "min_delta_score_den": 100},
                "max_promotions_per_checkpoint": 2,
                "max_patch_bytes": 0,
                "max_patch_files_touched": 0,
            },
            "success_criteria": {
                # Window_tasks drives the coordinator's promotion cadence.
                "min_new_solves_over_baseline": 0,
                "rolling_window": {"window_tasks": 10, "min_windows": 1},
            ...
            "stop_conditions": [{"kind": "MAX_TASKS", "max_tasks": 12}],
        },
    }
...
def test_v4_0_smoke_omega_run_self_improvement_accepts_promotion_valid(tmp_path: Path, repo_root: Path) -> None:
    ...
    # Verifier hard requirement when self-improvement is enabled: at least one promotion bundle exists.
    bundles = list((out_dir / "meta-core" / "store" / "bundles").glob("*"))
    assert bundles, "expected at least one promotion bundle"
