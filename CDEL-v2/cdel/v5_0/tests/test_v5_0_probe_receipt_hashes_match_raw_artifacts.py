...
def test_probe_receipt_hashes_match_raw_artifacts() -> None:
    repo_root = _repo_root()
    run_root = repo_root / "runs" / f"_pytest_thermo_v5_0_receipt_hash_{int(time.time()*1000)}"
    try:
        ...
        receipt = {
            ...
            "powermetrics_raw_hash": "sha256:" + "0" * 64,  # wrong on purpose
        ...
        with pytest.raises(Exception) as excinfo:
            verify(run_root)
        assert "CANON_HASH_MISMATCH" in str(excinfo.value)
    finally:
