...
def test_smoke_thermo_run_promotion_accepts_valid() -> None:
    repo_root = _repo_root()
    run_root = repo_root / "runs" / f"_pytest_thermo_v5_0_accept_{int(time.time()*1000)}"
    pack = _make_pack(enabled=True, max_promos=1, sealed_cfg_rel="CDEL-v2/cdel/v5_0/tests/fixtures/sealed_thermo_fixture_ok.toml")
    ...
    try:
        # Wait until a promotion bundle exists, then stop.
        for _ in range(200):
            bundles = list((run_root / "thermo" / "improvement" / "promotion_bundles").glob("*.json")) if (run_root / "thermo" / "improvement" / "promotion_bundles").exists() else []
            if bundles:
                break
            time.sleep(0.05)
