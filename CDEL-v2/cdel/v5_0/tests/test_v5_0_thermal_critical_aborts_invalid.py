...
def test_thermal_critical_aborts_invalid() -> None:
    repo_root = _repo_root()
    run_root = repo_root / "runs" / f"_pytest_thermo_v5_0_critical_{int(time.time()*1000)}"
    try:
        ...
        receipt = {
            ...
            "probe_status": "ABORT_THERMAL_CRITICAL",
        ...
        append("THERMO_PROBE_END", {"probe_receipt_path": "@ROOT/thermo/probes/" + receipt_path.name, "probe_receipt_hash": receipt["receipt_hash"], "probe_status": "ABORT_THERMAL_CRITICAL"})
        ...
        with pytest.raises(Exception) as excinfo:
            verify(run_root)
        assert "THERMO_THERMAL_CRITICAL_FATAL" in str(excinfo.value)
    finally:
