...
def test_unbounded_stop_provenance_required() -> None:
    repo_root = _repo_root()
    run_root = repo_root / "runs" / f"_pytest_thermo_v5_0_stopreq_{int(time.time()*1000)}"
    try:
        ...
        pm_path = run_root / "thermo" / "probes" / "raw" / "sha256_ok.powermetrics.txt"
        pm_path.write_text("Combined Power (CPU + GPU + ANE): 1000 mW\n", encoding="utf-8")
        ...
        append("THERMO_PROBE_END", {"probe_receipt_path": "@ROOT/thermo/probes/" + receipt_path.name, "probe_receipt_hash": receipt["receipt_hash"], "probe_status": "VALID"})

        # Stop file intentionally missing.
        append("THERMO_STOP", {"stop_kind": "EXTERNAL_SIGNAL", "stop_provenance_path": "@ROOT/thermo/STOP", "stop_provenance_hash": "sha256:" + "0" * 64})
