"""Daemon ledger parsing + hash chain validation (v6.0)."""
...
def load_daemon_ledger(path: Path) -> list[dict[str, Any]]:
    if not path.exists():
...
def truncate_incomplete_tail(path: Path) -> bool:
    ...
    # Validate all complete lines before the tail.
    for raw in lines[:-1]:
        try:
            load_canon_json_from_line(raw.decode("utf-8"))
        except UnicodeDecodeError as exc:  # noqa: BLE001
            raise CanonError("invalid json") from exc

    # Tail may be incomplete or missing newline; truncate it.
    truncated = b"\n".join(lines[:-1])
    if truncated and not truncated.endswith(b"\n"):
        truncated += b"\n"
