"""Superego ledger parsing + hash chain validation (v7.0)."""
...
def validate_superego_chain(entries: Iterable[dict[str, Any]]) -> tuple[str, int, int]:
    prev_hash = "GENESIS"
    prev_seq = 0
    ...
    for entry in entries:
        ...
        if tick < 0:
            _fail("SCHEMA_INVALID")
        if seq != prev_seq + 1:
            _fail("SUPEREGO_LEDGER_SEQ_MISMATCH")

        if prev_tick is None:
            prev_tick = tick
        else:
            if tick < prev_tick:
                _fail("SUPEREGO_LEDGER_TICK_REGRESSION")
            prev_tick = tick
        ...
        if prev_entry_hash != prev_hash:
            _fail("SUPEREGO_LEDGER_HASH_MISMATCH")
...
def load_superego_ledger(path: Path) -> list[dict[str, Any]]:
    if not path.exists():
...
__all__ = [
    ...
    "load_superego_ledger",
    "validate_superego_chain",
]
