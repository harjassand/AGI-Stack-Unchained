...
def test_v7_0_boundless_locked_without_enable(tmp_path: Path) -> None:
    ...
    req = build_request(
        {
            ...
            "objective_text": "explore",
            "capabilities": ["FS_READ_WORKSPACE", "FS_WRITE_RUNS_NEW", "NETWORK_NONE"],
            "target_paths": ["/Users/harjas/AGI-Stack-Clean /runs/"],
            "sealed_eval_required": False,
        }
    )

    prev = "GENESIS"
    entries = [build_entry(1, 0, "BOOT", prev)]
    prev = entries[-1]["entry_hash"]
    entries.append(build_entry(2, 1, "TICK_BEGIN", prev))
    prev = entries[-1]["entry_hash"]
    entries.append(build_entry(3, 1, "SUPEREGO_REQUEST", prev, {"request_id": req["request_id"], "objective_class": "BOUNDLESS_RESEARCH"}))
    prev = entries[-1]["entry_hash"]
    entries.append(
        build_entry(
            4,
            ...
            "SUPEREGO_DECISION",
            prev,
            {"request_id": req["request_id"], "decision": "ALLOW", "policy_hash": policy_hash()},
        )
    )
    prev = entries[-1]["entry_hash"]
    entries.append(
        build_entry(
            ...
            "ACTION_EXECUTED",
            prev,
            {"request_id": req["request_id"], "objective_class": "BOUNDLESS_RESEARCH"},
        )
    )
    prev = entries[-1]["entry_hash"]
    entries.append(build_entry(6, 1, "CHECKPOINT", prev))
