...
def test_v9_0_hazard_gating_enforced(tmp_path: Path) -> None:
    state = build_valid_state(tmp_path)
    record_path = state["attempt_dir"] / "attempt_record_v1.json"
    record = load_canon_json(record_path)
    record["hazard_class"] = "H2_RESTRICTED_DUAL_USE"
    write_canon_json(record_path, record)
    with pytest.raises(CanonError, match="SCIENCE_HAZARD_VIOLATION"):
