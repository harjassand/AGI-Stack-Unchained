...
ROOT="${ROOT:-.}"
RUNS_BASE="${RUNS_BASE:-runs_gate_mid}"
ANALYSIS_BASE="${ANALYSIS_BASE:-analysis_gate_mid}"
...
python3 experiments/run_matrix.py --matrix experiments/matrix_mid.json --out "$RUNS_BASE" --overwrite --root "$ROOT" $RESUME_FLAG
...
python3 analysis/check_claims.py --runs "$RUNS_BASE" --out "$ANALYSIS_BASE/claims_report.json" --suite mid
