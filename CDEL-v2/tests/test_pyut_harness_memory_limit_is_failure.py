...
@pytest.mark.skipif(
    getattr(__import__("os"), "name", "") != "posix", reason="resource limits unavailable"
)
def test_pyut_harness_memory_limit_is_failure(tmp_path):
    ...
    assert row["candidate_error"] in {"mem_limit", "timeout", "runtime_error"}
