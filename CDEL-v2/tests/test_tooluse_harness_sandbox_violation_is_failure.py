...
def _write_suite(cfg) -> str:
    row = {
        "episode": 0,
        "task_id": "sandbox_violation_v1",
        "max_steps": 1,
        "allowed_tools": ["read_file"],
        "initial_fs": [{"path": "ok.txt", "contents": "hi"}],
        "tool_calls": [{"tool": "read_file", "args": ["../secret.txt"]}],
        "success": {"type": "file_exists", "path": "ok.txt"},
    }
...
def test_tooluse_harness_sandbox_violation_is_failure(tmp_path):
    ...
    assert row["candidate_termination"] == "security_violation"
