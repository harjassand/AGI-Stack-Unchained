...
def main() -> int:
    parser = argparse.ArgumentParser(description="Genesis end-to-end v0.7 run")
    parser.add_argument("--system-config", default="genesis/configs/system_v0_7.json")
    ...
    ledger_dir = Path(os.getenv("LEDGER_DIR", "genesis/.cdel_ledger_e2e_v0_7"))
    receipts_dir = Path(config.get("receipts_dir", "genesis/receipts_v0_7"))
    component_store_dir = Path(config.get("component_store_dir", "genesis/components_v0_7"))
    calibration_path = Path(config.get("shadow_calibration_path", "genesis/shadow_calibration_v0_7.json"))
    ...
    nontrivial_fail = any(record.get("nontriviality_pass") is False for record in records)
    if not nontrivial_fail:
        raise SystemExit("expected at least one nontriviality failure in run log")
