...
def main() -> int:
    parser = argparse.ArgumentParser(description="Genesis end-to-end v0.9 run")
    parser.add_argument("--system-config", default="genesis/configs/system_v0_9.json")
    ...
    ledger_dir = Path(os.getenv("LEDGER_DIR", "genesis/.cdel_ledger_e2e_v0_9"))
    receipts_dir = Path(config.get("receipts_dir", "genesis/receipts_v0_9"))
    component_store_dir = Path(config.get("component_store_dir", "genesis/components_v0_9"))
    calibration_path = Path(config.get("shadow_calibration_path", "genesis/shadow_calibration_v0_9.json"))
    ...
    pattern_ids = [record.get("failure_pattern_id") for record in records if record.get("failure_pattern_id")]
    if not pattern_ids:
        raise SystemExit("expected at least one failure_pattern_id in run log")

    first_id = pattern_ids[0]
    total = sum(1 for record in records if record.get("failure_pattern_id") == first_id)
    if total > 1:
        first_idx = next(
            idx for idx, record in enumerate(records) if record.get("failure_pattern_id") == first_id
        )
        tail = records[first_idx:]
        half = len(tail) // 2
        first_half = sum(
            1 for record in tail[:half] if record.get("failure_pattern_id") == first_id
        )
        second_half = sum(
            1 for record in tail[half:] if record.get("failure_pattern_id") == first_id
        )
        if second_half > first_half:
            raise SystemExit("failure pattern recurrence did not decrease")
