...
def _predict(model_spec: dict, features: List[float]) -> float:
    ...
    score = _dot(weights, features) + bias
...
def _compute_metrics(model_spec: dict, policy_spec: dict, rows: list[dict]) -> tuple[float, float, int, list[int]]:
    episodes = {}
    actions: list[int] = []
    for row in rows:
        ...
        if not isinstance(obs, list) or label is None:
            continue
        pred = _predict(model_spec, obs)
        if pred < 0.0:
            pred = 0.0
        if pred > 1.0:
            pred = 1.0
        action = _act(policy_spec, [pred])
...
def evaluate_shadow_system(
    ...
    if system_capsule.get("artifact_type") != "ALGORITHM":
        trace = ShadowSystemTrace(status="VIOLATION", failing_test=None, counterexample=None)
        return ShadowSystemResult(
            "FAIL",
            "VIOLATION",
            None,
            None,
    ...
    env_key = env_id or (system_capsule.get("x-system") or {}).get("env_id", "policy_env_tiny")
    ...
    return_value, cost_value, n, actions = _compute_metrics(model_spec, policy_spec, rows)
    confidence = (system_capsule.get("contract", {}).get("statistical_spec", {}).get("confidence_requirement") or {}).get(
        "delta", 0.01
    )
    try:
    ...
    metrics = system_capsule.get("contract", {}).get("statistical_spec", {}).get("metrics") or []
    ...
    baseline_return = registry.get("SYSTEM", "policy_return")
    baseline_cost = registry.get("SYSTEM", "policy_cost")
    ...
    if decision == "PASS" and forager_max_tests > 0:
        ...
        tests_total += forager_count
        if tests:
            f_return, f_cost, _, _ = _compute_metrics(model_spec, policy_spec, tests)
            if f_return < return_threshold + margin:
                trace = ShadowSystemTrace(status="TEST_FAIL", failing_test="forager_return", counterexample=tests[0])
            ...
            if f_cost > cost_threshold - margin:
                trace = ShadowSystemTrace(status="TEST_FAIL", failing_test="forager_cost", counterexample=tests[0])
                duration_ms = int((time.time() - start) * 1000)
                return ShadowSystemResult(
                    "FAIL",
                    "TEST_FAIL",
    ...
    if not nontriviality_pass:
        ...
        trace = ShadowSystemTrace(status="TEST_FAIL", failing_test=failing, counterexample=None)
        return ShadowSystemResult(
            "FAIL",
            "FAIL",
            return_value,
    ...
    trace = ShadowSystemTrace(
        status="FAIL",
        failing_test="policy_return" if not return_ok else "policy_cost",
        counterexample={"env_id": env_key, "seed": seed},
    )
    return ShadowSystemResult(
        "FAIL",
        "FAIL",
