...
from genesis.core.codesign import run_codesign, SystemEvent  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Genesis system run")
    ...
    results = run_codesign(config)
    events: list[SystemEvent] = results["events"]
    ...
    for idx, event in enumerate(events):
        system_capsule = event.system_capsule
        shadow = event.shadow
        shadow_pass = shadow.decision == "PASS"
        if shadow_pass:
            summary["shadow_pass"] += 1

        policy_hash = system_capsule.get("x-system", {}).get("components", {}).get("policy", {}).get("hash", "")
        world_model_hash = system_capsule.get("x-system", {}).get("components", {}).get("world_model", {}).get("hash", "")
        record = {
            "artifact_type": "SYSTEM",
            "system_hash": capsule_hash(system_capsule),
            "component_hashes": {"policy": policy_hash, "world_model": world_model_hash},
            ...
            "failure_patterns_top": event.failure_patterns_top or [],
            "epoch_id": epoch_id,
            "model_family": event.descriptor.get("model_family"),
    ...
    summary_path = Path("genesis_system_summary.json")
