...
def verify_vectors() -> int:
    ...
    transcript_hash = sha256_hex(canonical_bytes(transcript))
    ...
    print("canonicalization vectors OK")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="GCJ-1 canonicalization tool")
    ...
    parser.add_argument("--capsule", type=str, help="capsule JSON path to hash")
    parser.add_argument("--receipt", type=str, help="receipt JSON path to hash")
    ...
    if args.capsule:
        ...
        print(compute_capsule_hash(capsule))
        return 0

    if args.receipt:
        receipt = load_json(Path(args.receipt))
        print(compute_receipt_hash(receipt))
        return 0
