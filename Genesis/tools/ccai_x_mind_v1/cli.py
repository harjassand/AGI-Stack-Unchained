...
if __package__ is None:
    ...
    from ccai_x_mind_v1.canonical_json import assert_no_floats
    from ccai_x_mind_v1.candidate_tar import build_candidate_tar
    from ccai_x_mind_v1.hashes import (
        candidate_id_from_tar,
        do_payload_hash,
        intervention_log_link_hash,
        mechanism_hash,
    )
    from ccai_x_mind_v1.validate_instance import load_json_strict, validate_path
else:
...
def _cmd_build_candidate(args: argparse.Namespace) -> int:
    try:
        ...
        candidate_id = build_candidate_tar(out_path, Path(args.artifact_dir))
        if args.print_id:
            print(candidate_id)
        return 0
    except Exception as exc:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="ccai-x-mind", description="CCAI-X Mind v1 validation and hashing tools"
    )
    ...
    build = sub.add_parser("build-candidate", help="build candidate tar from artifact dir")
    build.add_argument("--artifact-dir", required=True)
    build.add_argument("--out", required=True)
    build.add_argument("--print-id", action="store_true")
