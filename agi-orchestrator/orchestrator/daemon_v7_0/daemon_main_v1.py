...
from cdel.v7_0.daemon_state import compute_daemon_id, compute_snapshot_hash, load_snapshot  # noqa: E402
from cdel.v7_0.daemon_ledger import load_daemon_ledger, validate_daemon_chain  # noqa: E402
from cdel.v6_0.daemon_ledger import truncate_incomplete_tail  # noqa: E402

from ..superego_v7_0.alignment_runner_v1 import run_alignment  # noqa: E402
...
def _meta_hash() -> str:
    meta_root = _meta_core_root()
    return _read_text(meta_root / "meta_constitution" / "v7_0" / "META_HASH")
...
def _load_pack(pack_path: Path, config_dir: Path) -> dict[str, Any]:
    ...
    config_pack_path = config_dir / "rsi_daemon_pack_v7.json"
...
def _write_service_manifest(config_dir: Path, *, invocation: str, plist_path: str | None) -> None:
    ...
    if plist_path_resolved is None:
        default_plist = Path.home() / "Library" / "LaunchAgents" / "com.agi-stack.rsi-daemon.v7_0.plist"
...
def _run_activity(*, daemon_id: str, tick: int, activity: dict[str, Any], runs_root: Path) -> tuple[str, str]:
    ...
    receipt_hash = sha256_prefixed(canon_bytes(receipt))
    run_dir = runs_root / f"rsi_daemon_v7_0_tick_{tick}"
