"""Main loop for RSI daemon v8.0 math."""
...
from cdel.v8_0.daemon_state import compute_daemon_id, compute_snapshot_hash, load_snapshot  # noqa: E402
from cdel.v8_0.daemon_ledger import load_daemon_ledger, validate_daemon_chain  # noqa: E402
from cdel.v6_0.daemon_ledger import truncate_incomplete_tail  # noqa: E402
from cdel.v8_0.math_toolchain import load_toolchain_manifest, compute_manifest_hash

from ..superego_v8_0.alignment_runner_v1 import run_alignment  # noqa: E402
from ..superego_v7_0.capability_enforcer_v1 import enforce_capabilities  # noqa: E402
from ..superego_v7_0.request_builder_v1 import build_action_request  # noqa: E402
from ..superego_v7_0.superego_gate_v1 import evaluate_superego  # noqa: E402

from ..boundless_math_v8_0.budget_v1 import MathBudget  # noqa: E402
from ..boundless_math_v8_0.controller_v1 import BoundlessMathError, run_tick as run_math_tick  # noqa: E402
...
def _meta_hash() -> str:
    meta_root = _meta_core_root()
    return _read_text(meta_root / "meta_constitution" / "v8_0" / "META_HASH")
...
def _ensure_dirs(daemon_root: Path) -> dict[str, Path]:
    ...
    dirs = {
        ...
        "alignment_policy": alignment_dir / "policy",
        "alignment_ledger": alignment_dir / "ledger",
        "alignment_reports": alignment_dir / "reports",
        "alignment_clearance": alignment_dir / "clearance",
        "alignment_checkpoints": alignment_dir / "checkpoints",
        "math": state_dir / "math",
        "math_problems": state_dir / "math" / "problems",
        "math_work": state_dir / "math" / "work",
        "math_work_attempts": state_dir / "math" / "work" / "attempts",
        "math_ledger": state_dir / "math" / "ledger",
        "math_attempts": state_dir / "math" / "attempts",
        "math_attempts_receipts": state_dir / "math" / "attempts" / "receipts",
        "math_attempts_records": state_dir / "math" / "attempts" / "records",
        "math_attempts_proofs": state_dir / "math" / "attempts" / "proofs",
        "math_attempts_logs": state_dir / "math" / "attempts" / "logs",
        "math_attempts_sealed": state_dir / "math" / "attempts" / "sealed",
        "math_solved": state_dir / "math" / "solved",
        "math_solved_receipts": state_dir / "math" / "solved" / "receipts",
...
def _load_pack(pack_path: Path, config_dir: Path) -> dict[str, Any]:
    ...
    config_pack_path = config_dir / "rsi_daemon_pack_v8.json"
...
def _copy_canon_json(src: Path, dst: Path) -> dict[str, Any]:
    if not src.exists():
        raise CanonError('MISSING_ARTIFACT')
    payload = load_canon_json(src)
    if dst.exists():
        existing = load_canon_json(dst)
        if canon_bytes(existing) != canon_bytes(payload):
            raise CanonError('CANON_HASH_MISMATCH')
    else:
...
def _write_service_manifest(config_dir: Path, *, invocation: str, plist_path: str | None) -> None:
    ...
    if plist_path_resolved is None:
        default_plist = Path.home() / "Library" / "LaunchAgents" / "com.agi-stack.rsi-daemon.v8_0_math.plist"
...
def _run_activity(*, daemon_id: str, tick: int, activity: dict[str, Any], runs_root: Path) -> tuple[str, str]:
    ...
    receipt_hash = sha256_prefixed(canon_bytes(receipt))
    run_dir = runs_root / f"rsi_daemon_v8_0_math_tick_{tick}"
...
def run_daemon(pack_path: Path, daemon_root: Path, *, foreground: bool, service_plist: str | None) -> int:
    ...
    try:
        ...
        _daemon_id_ok(pack)
        math_cfg = dict(pack.get("math_boundless") or {})
        math_pack_src = Path(str(math_cfg.get("boundless_pack_path", "")))
        toolchain_src = Path(str(math_cfg.get("toolchain_manifest_path", "")))
        math_pack = _copy_canon_json(math_pack_src, dirs["config"] / "rsi_boundless_math_pack_v1.json")
        toolchain_manifest = _copy_canon_json(toolchain_src, dirs["config"] / "math_toolchain_manifest_v1.json")
        toolchain_manifest = load_toolchain_manifest(dirs["config"] / "math_toolchain_manifest_v1.json")
        ...
        alignment_cfg = dict(pack.get("alignment") or {})
        alignment_pack_path = Path(str(alignment_cfg.get("alignment_pack_path", "")))
        clearance_required_research = bool(alignment_cfg.get("clearance_required_for_research_bounded", True))
        clearance_required_boundless = bool(alignment_cfg.get("clearance_required_for_boundless", True))
        require_enable_boundless = bool(alignment_cfg.get("require_enable_research_file_for_boundless", True))
        clearance_refresh_ticks = int(alignment_cfg.get("clearance_refresh_ticks", 1))

        try:
            if alignment_pack_path.exists():
                run_alignment(alignment_pack_path=alignment_pack_path, out_alignment_dir=dirs["alignment"], issued_at_tick=current_tick)
                writer.append(event_type="ALIGNMENT_CLEARANCE_REFRESH", event_payload={}, tick=current_tick)
        except Exception:
            alignment_blocked = True
            alignment_reason = "PAUSED_ALIGNMENT_FAIL"
        ...
        while True:
            ...
            if alignment_blocked:
                if not paused:
                    writer.append(event_type="PAUSED", event_payload={"reason": alignment_reason or "PAUSED_ALIGNMENT_FAIL"}, tick=tick_engine.current_tick)
                    paused = True
                write_health_report(
                    dirs["health"] / "daemon_health_report_v1.json",
                    daemon_id=pack["daemon_id"],
                    ...
                    ledger_head_hash=writer.prev_hash,
                    status="PAUSED",
                    paused_reason=alignment_reason or "PAUSED_ALIGNMENT_FAIL",
                )
                time.sleep(0.1)
                continue
            ...
            if clearance_refresh_ticks > 0 and tick % clearance_refresh_ticks == 0 and alignment_pack_path.exists():
                try:
                    run_alignment(alignment_pack_path=alignment_pack_path, out_alignment_dir=dirs["alignment"], issued_at_tick=tick)
                    writer.append(event_type="ALIGNMENT_CLEARANCE_REFRESH", event_payload={}, tick=tick)
                    alignment_blocked = False
                    alignment_reason = None
                except Exception:
                    alignment_blocked = True
            ...
            if control.enable_research:
                writer.append(event_type="ENABLE_RESEARCH_PRESENT", event_payload={}, tick=tick)
            if control.enable_boundless_math:
                writer.append(event_type="ENABLE_BOUNDLESS_MATH_PRESENT", event_payload={}, tick=tick)

            activity = _select_activity(pack, tick=tick)
            objective_class = str(activity.get("objective_class") or "MAINTENANCE")
            capabilities = list(activity.get("capabilities") or [])
            target_paths = []
            if "FS_WRITE_DAEMON_STATE" in capabilities:
                target_paths.append(str(daemon_root / "state"))
            if "FS_WRITE_RUNS_NEW" in capabilities or not target_paths:
                target_paths.append(str(REPO_ROOT / "runs"))

            request = build_action_request(
                daemon_id=pack["daemon_id"],
                tick=tick,
                objective_class=objective_class,
                objective_text=str(activity.get("activity_id") or activity.get("activity_kind") or "activity"),
                capabilities=capabilities,
                target_paths=target_paths,
                sealed_eval_required=bool(activity.get("sealed_eval_required", False)),
            )
            writer.append(
                event_type="SUPEREGO_REQUEST",
                event_payload={
            ...
            try:
                decision_receipt, decision, reason = evaluate_superego(
                    alignment_dir=dirs["alignment"],
                    request=request,
                    daemon_id=pack["daemon_id"],
                    icore_id=pack["icore_id"],
                    meta_hash=pack["meta_hash"],
                    require_clearance_for_research=clearance_required_research,
                    require_clearance_for_boundless=clearance_required_boundless,
                    enable_research=control.enable_research if require_enable_boundless else True,
                )
            except Exception as exc:
                alignment_blocked = True
                alignment_reason = str(exc) or "PAUSED_ALIGNMENT_FAIL"
                writer.append(event_type="PAUSED", event_payload={"reason": alignment_reason}, tick=tick)
                continue
            writer.append(
                event_type="SUPEREGO_DECISION",
                event_payload={
                    "request_id": request["request_id"],
                    "decision": decision_receipt["decision"],
                    "policy_hash": decision_receipt["policy_hash"],
                    "decision_hash": decision_receipt["decision_hash"],
                    "decision_reason_code": decision_receipt["decision_reason_code"],
                },
                tick=tick,
            )
            ...
            if decision != "ALLOW":
                writer.append(
                    event_type="ACTION_SKIPPED_DENY",
                    event_payload={
                        "request_id": request["request_id"],
                        "objective_class": request["objective_class"],
                ...
                tick_engine.record_work_units(0)
                action_skipped = True
            else:
                try:
                    enforce_capabilities(request, agi_root=REPO_ROOT, daemon_root=daemon_root)
                except Exception:
                    writer.append(
                        event_type="ACTION_SKIPPED_DENY",
                        event_payload={
                            "request_id": request["request_id"],
                            "objective_class": request["objective_class"],
                            "decision_reason_code": "DAEMON_FORBIDDEN_CAPABILITY",
                        },
                        tick=tick,
                    )
                    tick_engine.record_work_units(0)
                    action_skipped = True
                if not action_skipped:
                    activity_kind = str(activity.get("activity_kind"))
                    if activity_kind == "MATH_BOUNDLESS_V1" and bool(math_cfg.get("enabled", False)):
                        if bool(math_cfg.get("require_enable_research_file", True)) and not control.enable_research:
                            writer.append(
                                event_type="BOUNDLESS_MATH_LOCKED_NO_ENABLE_RESEARCH",
                                event_payload={"request_id": request["request_id"]},
                                tick=tick,
                            )
                            writer.append(
                                event_type="ACTION_SKIPPED_DENY",
                                event_payload={
                                    "request_id": request["request_id"],
                                    "objective_class": request["objective_class"],
                                    "decision_reason_code": "BOUNDLESS_MATH_LOCKED_NO_ENABLE_RESEARCH",
                                },
                                tick=tick,
                            )
                            tick_engine.record_work_units(0)
                            action_skipped = True
                        elif bool(math_cfg.get("require_enable_boundless_math_file", True)) and not control.enable_boundless_math:
                            writer.append(
                                event_type="BOUNDLESS_MATH_LOCKED_NO_ENABLE_BOUNDLESS_MATH",
                                event_payload={"request_id": request["request_id"]},
                                tick=tick,
                            )
                            writer.append(
                                ...
                                event_payload={
                                    "request_id": request["request_id"],
                                    "objective_class": request["objective_class"],
                                    "decision_reason_code": "BOUNDLESS_MATH_LOCKED_NO_ENABLE_BOUNDLESS_MATH",
                                },
                                tick=tick,
                            )
                            tick_engine.record_work_units(0)
                            action_skipped = True
                        elif math_pack.get("toolchain_manifest_hash") != compute_manifest_hash(toolchain_manifest):
                            writer.append(
                                event_type="BOUNDLESS_MATH_TOOLCHAIN_DRIFT",
                                event_payload={"request_id": request["request_id"]},
                                tick=tick,
                            )
                            writer.append(
                                ...
                                event_payload={
                                    "request_id": request["request_id"],
                                    "objective_class": request["objective_class"],
                                    "decision_reason_code": "BOUNDLESS_MATH_TOOLCHAIN_DRIFT",
                                },
                                tick=tick,
                            )
                            tick_engine.record_work_units(0)
                            ...
                            budget = MathBudget(
                                attempts_today=tick_engine.counters.math_attempts_today,
                            ...
                            try:
                                receipt_hash, receipt_path, _attempt_receipt = run_math_tick(
                                    tick=tick,
                                    daemon_id=pack["daemon_id"],
                                    ...
                                    capabilities=request["capabilities"],
                                    budget=budget,
                                    daemon_ledger_writer=writer,
                                )
                            except BoundlessMathError as exc:
                                writer.append(
                                    event_type=str(exc),
                                    event_payload={"request_id": request["request_id"]},
                                    tick=tick,
                                ...
                                writer.append(
                                    ...
                                    event_payload={
                                        ...
                                        "activity_receipt_path": str(receipt_path),
                                    },
                                    tick=tick,
                                )
                                tick_engine.record_work_units(1)
                                tick_engine.record_math_attempt()
                    else:
