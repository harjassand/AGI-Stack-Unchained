...
payload = {
    "new_symbols": ["is_even_llm_retry"],
    "definitions": [
        {
            "name": "is_even_llm_retry",
    ...
    "declared_deps": [],
    "specs": [],
    "concepts": [{"concept": "algo.is_even", "symbol": "is_even_llm_retry"}],
}
...
retry_counts = [a.get("llm_retry_count") for a in attempts if a.get("llm_retry_count") is not None]
if not retry_counts:
    raise SystemExit("ERROR: expected llm_retry_count in manifest")
if max(retry_counts) < 1:
    raise SystemExit("ERROR: expected llm_retry_count > 0")
PY
