...
def _write_plan_skill(path: Path, *, plan_id: str, deps: list[str], domain: str, concept: str) -> None:
    payload = {
...
def test_env_agent_uses_plan_skill_when_available(tmp_path: Path) -> None:
    plan_dir = tmp_path / "plan_skills"
    plan_dir.mkdir()
    _write_plan_skill(plan_dir / "plan.json", plan_id="plan-1", deps=["policy_special"], domain="env", concept="gridworld")
    ...
    candidates = proposer.propose(context=bundle, budget=1, rng_seed=7)
    assert candidates
    candidate = candidates[0]
    assert candidate.payload.get("declared_deps") == ["policy_special"]
    assert candidate.meta is not None
    assert candidate.meta.get("plan_skill_used") is True
    assert candidate.meta.get("plan_skill_selected") == "plan-1"
