...
def test_scoreboard_written_and_schema(tmp_path: Path) -> None:
    client = CDELClient()
    ...
    required = {
        ...
        "best_candidate_success_rate",
        "diff_sum_distribution",
        "error_counts",
        "heldout_cert_passed",
        "llm",
    }
    assert required.issubset(set(scoreboard))
