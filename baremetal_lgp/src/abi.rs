pub const SCRATCH_WORDS: usize = 16_384; // 64KB / 4
...
// complex scalar regs are paired float regs: creg k uses f[2k], f[2k+1]
pub const C_REGS: usize = 8;

pub const CALLSTACK_DEPTH: usize = 1024;

pub const CONST_POOL_WORDS: usize = 128; // fixed K
pub const LIB_SLOTS: usize = 256;
pub const LIB_MAX_INSNS: usize = 1024;

// Candidate limits (must be enforced by cfg::verify)
pub const CAND_MAX_BLOCKS: usize = 256;
pub const CAND_MAX_INSNS: usize = 4096; // total instr words after link
pub const BLOCK_MAX_INSNS: usize = 1024; // per block safeguard

// meta ABI indices (u32)
...
// reserved meta slots for regime/task parameters
pub const META_P0: usize = 6; // family-specific
