...
pub trait GraphBackendAdapter {
    ...
    fn run_plan(&self, plan: &GraphExecPlan, window: &[u8], prog: &ScirV2Program) -> Result<Vec<u16>>;
}
...
impl GraphBackendAdapter for Apf3GraphBackendAdapter {
    fn lower_program(&self, prog: &ScirV2Program) -> Result<GraphExecPlan> {
        let graph_safe = prog.core_blocks.iter().flat_map(|b| b.ops.iter()).all(|op| {
            matches!(
                op.op.as_str(),
                "LinearMix"
                    | "AffineBias"
        ...
        Ok(GraphExecPlan {
            plan_hash: crate::apfsc::artifacts::digest_json(&(op_count, prog.schedule.scheduler_class))?,
            op_count,
        })
    }

    fn run_plan(&self, _plan: &GraphExecPlan, window: &[u8], prog: &ScirV2Program) -> Result<Vec<u16>> {
