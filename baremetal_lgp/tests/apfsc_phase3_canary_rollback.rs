...
fn pcold_canary_fail_keeps_active_and_rollback_pointer() {
    ...
    let incumbent =
        baremetal_lgp::apfsc::candidate::load_candidate(&root, &incumbent_hash).expect("incumbent");
    ...
    assert!(!receipt.pass);
    let active_after = baremetal_lgp::apfsc::artifacts::read_pointer(&root, "active_candidate")
        .expect("active after");
    let rollback_after = baremetal_lgp::apfsc::artifacts::read_pointer(&root, "rollback_candidate")
        .expect("rollback after");
