#!/usr/bin/env python3
"""Deterministically regenerate immutable_core_lock_v1.json for meta_constitution/v4_0.

This is a small, local stand-in for the "kernel exclude semantics" lock builder.
It scans a fixed set of source roots, applies a small exclude list (dirs + globs),
and writes a content-addressed lock file matching the v2_3 immutable-core format.

Notes:
- The lock intentionally excludes META_HASH and the lock file itself to avoid
...
OUT_PATH = REPO_ROOT / "meta-core" / "meta_constitution" / "v4_0" / "immutable_core_lock_v1.json"

DEFAULT_SOURCE_ROOTS = [
    "meta-core/kernel/verifier/",
    "meta-core/meta_constitution/v4_0/",
    "Genesis/schema/v4_0/",
    "CDEL-v2/cdel/v4_0/",
    "CDEL-v2/cdel/v3_3/",
    "CDEL-v2/cdel/kernel/",
    "Extension-1/agi-orchestrator/orchestrator/omega_v4_0/",
    "Extension-1/agi-orchestrator/orchestrator/swarm_v3_3/",
]

# Mirrors meta-core/kernel exclude semantics currently used across the repo.
DEFAULT_EXCLUDES = [
    ".git/",
    ".venv/",
    ".pytest_cache/",
...
def _should_exclude(relpath: str, *, excludes: list[str]) -> bool:
    # Exclude output lock itself regardless of patterns.
    ...
    for pat in excludes:
        if pat.endswith("/"):
            ...
            if d in parts:
                return True
            # Also allow prefix-style excludes like "runs/" at repo root.
            if relpath.startswith(pat):
                return True
            continue

        # Filename or relpath glob excludes (e.g. *.pyc).
...
def _iter_files(repo_root: Path, *, source_roots: list[str], excludes: list[str]) -> list[FileEntry]:
    ...
    for root_rel in source_roots:
        ...
        for dirpath, dirnames, filenames in os.walk(root_path):
            ...
            dirnames[:] = [d for d in dirnames if not _should_exclude((dirpath_p / d).relative_to(repo_root).as_posix(), excludes=excludes)]
...
def main() -> None:
    ...
    # Compute lock_id with lock_head_hash placeholder semantics.
    tmp = dict(lock)
