...
from constants import NULL_BUNDLE_HASH, FAILPOINT_ENV, FAILPOINT_AFTER_PREV_WRITE  # noqa: E402
...
class TestAtomicitySimulatedFailure(unittest.TestCase):
    def test_failpoint_after_prev_write(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            os.environ[FAILPOINT_ENV] = FAILPOINT_AFTER_PREV_WRITE
            try:
                exit_code, out = apply_bundle(meta_root, bundle_dir)
            finally:
            ...
            if os.path.isfile(active_path):
                with open(active_path, "r", encoding="utf-8") as f:
                    active_hash = f.read().strip()
                self.assertEqual(active_hash, parent_hash)

            prev_path = os.path.join(meta_root, "active", "PREV_ACTIVE_BUNDLE")
            if os.path.isfile(prev_path):
                with open(prev_path, "r", encoding="utf-8") as f:
                    prev_hash = f.read().strip()
                self.assertIn(prev_hash, {parent_hash, NULL_BUNDLE_HASH})

            audit_code, audit_out = audit_active(meta_root)
            self.assertIn(audit_code, {0, 2})
            if audit_code == 0:
                self.assertEqual(audit_out.get("verdict"), "OK")
            else:
                self.assertEqual(audit_out.get("verdict"), "INVALID")

            store_dir = os.path.join(meta_root, "store", "bundles")
            if os.path.isdir(store_dir):
                self.assertTrue(os.listdir(store_dir))
