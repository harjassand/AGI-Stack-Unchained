...
PACK_PATH = REPO_ROOT / "campaigns" / "rsi_omega_daemon_v19_0" / "rsi_omega_daemon_pack_v1.json"
...
def test_v19_pack_registry_wiring_smoke() -> None:
    ...
    for key, value in pack.items():
        ...
        assert target.exists(), f"pack field {key} points to missing path: {target}"

    registry_rel = str(pack.get("omega_capability_registry_rel", "")).strip()
    assert registry_rel, "missing omega_capability_registry_rel in pack"
    registry_path = (pack_dir / registry_rel).resolve()
    assert registry_path.exists(), f"registry path is missing: {registry_path}"

    registry = _load_json(registry_path)
    capabilities = registry.get("capabilities")
    assert isinstance(capabilities, list), "registry.capabilities must be a list"

    for row in capabilities:
        assert isinstance(row, dict), "capability entries must be objects"
