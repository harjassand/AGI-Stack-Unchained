"""Coordinator for RSI Omega daemon v18.0 tick execution."""
...
from .applier_v1 import run_activation
from .decider_v1 import decide
from .diagnoser_v1 import diagnose
from .dispatcher_v1 import dispatch_campaign
from .goal_synthesizer_v1 import synthesize_goal_queue
from .io_v1 import freeze_pack_config, load_goal_queue, write_goal_queue_effective
from .locks_v1 import acquire_lock
from .observer_v1 import observe, read_meta_core_active_manifest_hash
...
def _load_prev_tick_perf(prev_state_dir: Path | None) -> tuple[dict[str, Any] | None, dict[str, str] | None]:
    ...
    for perf_dir in candidates:
        ...
        run_id = _run_id_from_state_dir(perf_dir)
...
def _load_prev_observation(prev_state_dir: Path | None) -> tuple[dict[str, Any] | None, dict[str, str] | None]:
    ...
    for obs_dir in candidates:
        ...
        if best_payload is None:
            continue
        source: dict[str, str] | None = None
        run_id = _run_id_from_state_dir(obs_dir)
        if run_id is not None:
            source = {
...
def run_tick(
    *,
    ...
    daemon_root = run_root / "daemon" / "rsi_omega_daemon_v18_0"
    ...
    with acquire_lock(lock_path):
        ...
        tick_total_ns = time.monotonic_ns() - tick_start_ns
        timings_line = " ".join(
            [
                f"tick_u64={int(tick_u64)}",
                *(f"{stage}_ns={int(stage_timings_ns[stage])}" for stage in [
                    "freeze_pack_config",
                    "observe",
                    "diagnose",
                    "decide",
