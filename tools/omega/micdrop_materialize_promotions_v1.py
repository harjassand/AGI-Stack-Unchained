#!/usr/bin/env python3
"""Materialize accepted micdrop promotions into the solver file."""
...
_CONST_RE = re.compile(r"^MICDROP_CAPABILITY_LEVEL\s*=\s*(\d+)\s*$", re.MULTILINE)
...
def _read_solver_level(content: str) -> int:
    marker_match = _MARKER_RE.search(content)
    const_match = _CONST_RE.search(content)
    if marker_match is None or const_match is None:
        raise RuntimeError("solver marker lines are missing")
    marker_level = int(marker_match.group(1))
    const_level = int(const_match.group(1))
    if marker_level != const_level:
        raise RuntimeError("solver capability marker mismatch")
    return marker_level


def _rewrite_solver_level(content: str, target_level: int) -> str:
    next_text, marker_count = _MARKER_RE.subn(f"# MICDROP_CAPABILITY_LEVEL:{int(target_level)}", content, count=1)
    if marker_count != 1:
        raise RuntimeError("failed to rewrite solver capability marker comment")
    next_text, const_count = _CONST_RE.subn(f"MICDROP_CAPABILITY_LEVEL = {int(target_level)}", next_text, count=1)
    if const_count != 1:
        raise RuntimeError("failed to rewrite solver capability marker constant")
    return next_text
...
def main() -> int:
    ...
    if not solver_path.exists() or not solver_path.is_file():
        raise RuntimeError("solver file missing")

    content = solver_path.read_text(encoding="utf-8")
    prior_level = _read_solver_level(content)
    ...
    if target_level != prior_level:
        solver_path.write_text(_rewrite_solver_level(content, target_level), encoding="utf-8")
        materialized_b = True

    summary = {
        "schema_version": "micdrop_materialize_promotions_v1",
        "solver_path": solver_path.relative_to(_REPO_ROOT).as_posix(),
    ...
    write_canon_json(out_path, summary)
